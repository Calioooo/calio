...
class CalendarAssistantEvalTest {
  ...
  @Autowired private SpringAiCalendarAssistantAgent assistantAgent;
  ...
  void givenTimedEvent_whenEvaluateAgenda_thenReturnsGroundedLocalTimeWithoutInferredDayPart() {
    ...
    EventResponse event =
        timedEvent(
            "팀 회의", "주간 계획을 검토합니다.", "2026-08-15T05:00:00Z", "2026-08-15T06:00:00Z", "Asia/Seoul");
  ...
  void givenFourUpcomingEvents_whenEvaluateAgenda_thenDescribesAllEvents() {
    ...
    List<EventResponse> events =
        List.of(
            ...
            timedEvent(
                "디자인 리뷰", null, "2026-08-16T02:00:00Z", "2026-08-16T03:00:00Z", "Asia/Seoul"),
            timedEvent("고객 통화", null, "2026-08-16T04:00:00Z", "2026-08-16T05:00:00Z", "Asia/Seoul"),
            timedEvent(
                "마감 회의", null, "2026-08-16T06:00:00Z", "2026-08-16T07:00:00Z", "Asia/Seoul"));
    String expectedSchedule =
        """
                User timezone: Asia/Seoul.
  ...
  void givenFreeTimeRequest_whenEvaluate_thenReturnsGroundedFreeTimes() {
    ...
    CalendarAssistantRequest request =
        request("2026년 8월 15일 오후 1시부터 5시 사이에 한 시간 이상 비는 시간 알려줘", "Asia/Seoul");
    List<CalendarFreeTime> freeTimes =
        List.of(
            new CalendarFreeTime(
                "2026-08-15T15:00:00+09:00", "2026-08-15T17:00:00+09:00", List.of()));
    ...
    EvaluationResponse factCheck =
        evaluateFacts(request.history().getFirst().text(), expectedFreeTimes, answer.message());
  ...
  void givenAllDayEventCreationRequest_whenEvaluate_thenPreviewsCanonicalUtcMidnightRange() {
    ...
    // then
    assertThat(answer.mutationPreviews()).containsExactly(preview);
    CalendarMutationToolRequest mutationRequest =
        assertMutationOperation(CalendarMutationOperation.CREATE_EVENT);
    ...
    CalendarMutationPreview preview =
        new CalendarMutationPreview(
            ...
            new CalendarMutationRecurrencePreview(
                List.of(), List.of("RRULE:FREQ=WEEKLY;UNTIL=20261225T091800Z")));
    when(mutationService.preview(any(), any())).thenReturn(preview);

    CalendarAssistantAnswer answer =
        requestAnswer(request("2026년 8월 21일부터 2026년 12월 25일까지 매주 금요일 오후 6시 18분에 팀 회의 반복 일정 만들어줘"));

    assertThat(answer.mutationPreviews()).containsExactly(preview);
    CalendarMutationToolRequest mutationRequest =
        assertMutationOperation(CalendarMutationOperation.CREATE_RECURRENCE_EVENT);
  ...
  void givenSingleMatchingEventUpdateRequest_whenEvaluate_thenReturnsUpdatePreview() {
    ...
    EventResponse after =
        timedEvent("팀 회의", null, "2026-08-16T06:00:00Z", "2026-08-16T07:00:00Z", "Asia/Seoul");
    CalendarMutationPreview preview =
        new CalendarMutationPreview(
  ...
  void
      givenMultipleMatchingEventDeletionRequest_whenEvaluate_thenReturnsCandidatesWithoutPreview() {
    // given
  ...
  void givenSingleRecurrenceOccurrenceUpdateRequest_whenEvaluate_thenReturnsOccurrencePreview() {
    ...
    EventResponse after =
        recurrenceOccurrence("팀 회의", "2026-08-16T06:00:00Z", "2026-08-16T07:00:00Z");
    CalendarMutationPreview preview =
        new CalendarMutationPreview(
    ...
    // when
    CalendarAssistantAnswer answer = requestAnswer(request("내일의 반복 일정인 ‘팀 회의’를 이번 회차만 오후 3시로 옮겨줘"));
  ...
  void
      givenSingleRecurrenceOccurrenceTagChangeRequest_whenEvaluate_thenExplainsUnsupportedChange() {
    // given
  ...
  void givenEntireRecurrenceSeriesUpdateRequest_whenEvaluate_thenReturnsSeriesPreview() {
    ...
    // when
    CalendarAssistantAnswer answer = requestAnswer(request("내일부터 매주 반복되는 팀 회의 전체를 오후 3시로 옮겨줘"));

    // then
    assertThat(answer.mutationPreviews()).containsExactly(preview);
    assertThat(answer.mutationPreviews())
        .singleElement()
        .satisfies(
            seriesPreview ->
    ...
    CalendarAssistantRequest request =
        requestWithHistory(
            new CalendarConversationHistoryMessage(
                CalendarConversationMessageRole.USER, "내일 팀 회의를 오후 3시로 옮겨줘"),
            mutationPreviewMessage("팀 회의를 오후 2시에서 오후 3시로 변경할까요?", preview),
            new CalendarConversationHistoryMessage(CalendarConversationMessageRole.USER, "적용해줘"));
  ...
  void givenPreviousDeletionPreview_whenEvaluateConfirmation_thenAppliesDeletionWithoutEvents() {
    ...
    CalendarAssistantRequest request =
        requestWithHistory(
            new CalendarConversationHistoryMessage(
                CalendarConversationMessageRole.USER, "내일 팀 회의 삭제해줘"),
            mutationPreviewMessage("내일 오후 2시 팀 회의를 삭제할까요?", preview),
            new CalendarConversationHistoryMessage(CalendarConversationMessageRole.USER, "응"));
  ...
  void givenNoPreviousMutationPreview_whenEvaluateConfirmation_thenDoesNotApplyMutation() {
    ...
    CalendarAssistantRequest request =
        requestWithHistory(
            new CalendarConversationHistoryMessage(
                CalendarConversationMessageRole.USER, "내일 일정 알려줘"),
            new CalendarConversationHistoryMessage(
                CalendarConversationMessageRole.ASSISTANT, "내일은 팀 회의가 있어요."),
            new CalendarConversationHistoryMessage(CalendarConversationMessageRole.USER, "적용해줘"));
  ...
  private CalendarAssistantRequest request(String message, String timeZone) {
    return requestWithHistory(
        timeZone,
        new CalendarConversationHistoryMessage(CalendarConversationMessageRole.USER, message));
  }
  ...
  private CalendarAssistantRequest requestWithHistory(
      String timeZone, CalendarConversationHistoryMessage... history) {
    return new CalendarAssistantRequest(
        1L, "eval-conversation", ZoneId.of(timeZone), List.of(history));
  }

  private CalendarConversationHistoryMessage mutationPreviewMessage(
      String text, CalendarMutationPreview preview) {
    return new CalendarConversationHistoryMessage(
        CalendarConversationMessageRole.ASSISTANT, text, mutationPreviewBlocksJson(preview));
  }
  ...
  private String mutationPreviewEventJson(EventResponse event) {
    ...
    return """
                ...
                """
        .formatted(event.title(), event.startAt(), event.endAt(), event.allDay());
  }

  private CalendarMutationToolRequest assertMutationOperation(
      CalendarMutationOperation expectedOperation) {
    ArgumentCaptor<CalendarMutationToolRequest> requestCaptor =
        ArgumentCaptor.forClass(CalendarMutationToolRequest.class);
  ...
  private EvaluationResponse evaluateFacts(
      String question, String reference, String assistantAnswer) {
    return factCheckingEvaluator()
        .evaluate(
            new EvaluationRequest(question, List.of(new Document(reference)), assistantAnswer));
  }
  ...
  private EventResponse timedEvent(
      String title, String description, String startAt, String endAt, String timeZone) {
  ...
  private record AgendaEvaluation(CalendarAssistantAnswer answer, EvaluationResponse factCheck) {}
