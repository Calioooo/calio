...
@SpringBootTest(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:ai-calendar-conversation-service-test;MODE=MySQL;DB_CLOSE_ON_EXIT=FALSE",
...
class CalendarConversationServiceTest {
  ...
  @MockitoBean private SpringAiCalendarAssistantAgent assistantAgent;
  ...
  void givenTwentyOneMessages_whenRecordUserMessage_thenReturnsNewestTwentyInChronologicalOrder() {
    ...
    List<String> histories =
        IntStream.rangeClosed(1, 21)
            .mapToObj(
                index ->
                    conversationService.recordUserMessageAndGetRecentHistory(
                        conversationRecordId, "message-" + index))
    ...
    assertThat(histories)
        .containsExactlyElementsOf(
            IntStream.rangeClosed(2, 21).mapToObj(index -> "message-" + index).toList());
  }
  ...
  void
      givenInactiveConversation_whenDeleteInactiveConversations_thenDeletesConversationAndMessages() {
    // given
    Account account = accountRepository.saveAndFlush(new Account());
    String conversationId = conversationService.createConversation(account.getId());
    Long conversationRecordId = conversationRecordId(account.getId(), conversationId);
    ...
    // when
    int deletedCount =
        conversationService.deleteInactiveConversations(Instant.parse("2026-07-01T00:00:00Z"));
  ...
  void givenUnsupportedMessage_whenSendMessage_thenRecordsRandomScopeGuideWithoutCallingAgent() {
    ...
    // when
    var response =
        conversationService.sendMessage(account.getId(), conversationId, "너는 몇 살이야?", "Asia/Seoul");
        ...
        .satisfiesExactly(
            ...
            savedMessage -> {
              assertThat(savedMessage.getRole())
                  .isEqualTo(CalendarConversationMessageRole.ASSISTANT);
  ...
  void givenSupportedMessage_whenSendMessage_thenDelegatesToAgent() {
    ...
    when(assistantAgent.answer(any()))
        .thenReturn(
            new CalendarAssistantAnswer(
                "안녕하세요. 일정 조회와 빈 시간 찾기를 도와드릴게요.", List.of(), List.of(), List.of()));

    // when
    var response =
        conversationService.sendMessage(account.getId(), conversationId, "안녕", "Asia/Seoul");

    // then
    ArgumentCaptor<CalendarAssistantRequest> requestCaptor =
        ArgumentCaptor.forClass(CalendarAssistantRequest.class);
    ...
    assertThat(requestCaptor.getValue().history())
        .extracting(CalendarConversationHistoryMessage::text)
        .containsExactly("안녕");
    assertThat(messageRepository.findAll())
        .extracting(message -> message.getRole())
        .containsExactly(
            CalendarConversationMessageRole.USER, CalendarConversationMessageRole.ASSISTANT);
  }
  ...
  void givenAssistantResponseBlock_whenSendNextMessage_thenProvidesStoredBlockToAgent() {
    ...
    CalendarConversationHistoryMessage assistantMessage =
        requestCaptor.getAllValues().get(1).history().get(1);
    assertThat(assistantMessage.assistantResponseBlocksJson()).contains("\"type\":\"EVENTS\"");
  }
