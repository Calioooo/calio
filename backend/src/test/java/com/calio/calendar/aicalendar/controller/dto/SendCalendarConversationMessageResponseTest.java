...
class SendCalendarConversationMessageResponseTest {
  ...
  void givenAgentAnswerWithToolResults_whenCreateResponse_thenIncludesTypedBlocks() {
    ...
    assertThat(response.blocks().get(1).items())
        .isEqualTo(
            List.of(
                new FreeTimeResponse(
                    "2026-07-01T09:00:00Z", "2026-07-01T10:00:00Z", List.of("Holiday"))));
    assertThat(response.blocks().get(2).type())
        .isEqualTo(CalendarAssistantBlockType.MUTATION_PREVIEW);
    assertThat(response.blocks().get(2).items())
        .singleElement()
        .satisfies(
            preview -> {
              CalendarMutationPreviewResponse mutationPreview =
                  (CalendarMutationPreviewResponse) preview;
              ...
              assertThat(mutationPreview.after().startAt())
                  .isEqualTo(Instant.parse("2026-07-01T11:00:00Z"));
  ...
  void givenSeriesPreview_whenCreateResponse_thenIncludesRecurrenceRules() {
            ...
            List.of(
                new CalendarMutationPreview(
                    ...
                    new CalendarMutationRecurrencePreview(
                        List.of("RRULE:FREQ=WEEKLY;BYDAY=FR"), List.of("RRULE:FREQ=DAILY")))));
    ...
    // then
    CalendarMutationPreviewResponse preview =
        (CalendarMutationPreviewResponse) response.blocks().get(0).items().get(0);
