...
class CalendarConversationFlowEvalTest {
  ...
  @Autowired private CalendarConversationService conversationService;

  @Autowired private CalendarConversationQueryService conversationQueryService;

  @Autowired private AccountRepository accountRepository;

  @Autowired private CalendarConversationRepository conversationRepository;

  @Autowired private CalendarConversationMessageRepository messageRepository;
  ...
  void printConversationResponses(TestInfo testInfo) {
    ...
    System.out.printf(
        """
                ...
                responses: %s%n
                """,
        testInfo.getDisplayName(), responses);
  }

  @Test
  @DisplayName("생성에 종료 시각이 없으면 재질문 후 이전 맥락으로 생성 Preview를 만든다")
  void
      givenCreationWithoutEndTime_whenUserProvidesEndTime_thenCreatesPreviewFromConversationHistory() {
    // given
    ...
    // when
    SendCalendarConversationMessageResponse first =
        send(account, conversationId, "내일 오후 2시에 팀 회의 만들어줘");
    SendCalendarConversationMessageResponse second = send(account, conversationId, "오후 3시에 끝나");
    ...
    assertThat(first.blocks())
        .noneMatch(block -> block.type() == CalendarAssistantBlockType.MUTATION_PREVIEW);
    assertThat(second.assistantMessage()).isNotBlank();
    assertThat(second.blocks())
  ...
  void
      givenUpdateWithoutNewTime_whenUserProvidesNewTime_thenCreatesPreviewFromConversationHistory() {
    // given
    Account account = accountRepository.saveAndFlush(new Account());
    String conversationId = conversationService.createConversation(account.getId());
    ...
    assertThat(capturedPreviewRequest().operation())
        .isEqualTo(CalendarMutationOperation.UPDATE_EVENT);
    verify(mutationService, never()).apply(any(), any());
  }
  ...
  void
      givenRecurringUpdateWithoutScope_whenUserChoosesOccurrence_thenCreatesOccurrencePreviewFromConversationHistory() {
    // given
    Account account = accountRepository.saveAndFlush(new Account());
    String conversationId = conversationService.createConversation(account.getId());
    EventResponse before =
        recurrenceOccurrence(
            "팀 회의", "2026-08-16T05:00:00Z", "2026-08-16T05:00:00Z", "2026-08-16T06:00:00Z");
    EventResponse after =
        recurrenceOccurrence(
    ...
    // when
    SendCalendarConversationMessageResponse first =
        send(account, conversationId, "내일의 반복 일정인 팀 회의를 오후 3시로 옮겨줘");
  ...
  void givenMultipleMutationPreviews_whenUserConfirmsWithoutTarget_thenAsksWhichChangeToApply() {
    ...
    EventResponse movedTeamMeeting =
    ...
    when(mutationService.preview(eq(account.getId()), any()))
        .thenReturn(
            new CalendarMutationPreview(
                ...
                movedTeamMeeting),
            new CalendarMutationPreview(
                CalendarMutationType.DELETE, CalendarMutationScope.EVENT, designReview, null));

    // when
    SendCalendarConversationMessageResponse first =
        send(account, conversationId, "내일 팀 회의를 오후 3시부터 4시까지로 옮겨줘");
    SendCalendarConversationMessageResponse second =
        send(account, conversationId, "그리고 내일 디자인 리뷰는 삭제해줘");
    ...
    assertResponseMatchesReference(
        ...
        confirmation);
    assertThat(confirmation.blocks())
        .noneMatch(block -> block.type() == CalendarAssistantBlockType.MUTATION_PREVIEW);
    verify(mutationService, never()).apply(any(), any());
  }

  private SendCalendarConversationMessageResponse send(
      Account account, String conversationId, String message) {
    SendCalendarConversationMessageResponse response =
        conversationService.sendMessage(account.getId(), conversationId, message, "Asia/Seoul");
  ...
  private void assertResponseMatchesReference(
      String question, String expectedResponse, SendCalendarConversationMessageResponse response) {
  ...
  private void assertConversationHistory(
    ...
    assertThat(conversationQueryService.getRecentHistory(conversationRecordId, 20))
        ...
        .containsExactly(
            firstUserMessage, firstAssistantMessage, secondUserMessage, secondAssistantMessage);
  }
  ...
  private EventResponse recurrenceOccurrence(
      String title, String originStartAt, String startAt, String endAt) {
    return new EventResponse(
        null,
        title,
        null,
