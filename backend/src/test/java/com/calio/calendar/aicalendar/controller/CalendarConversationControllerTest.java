...
@SpringBootTest(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:ai-calendar-conversation-controller-test;MODE=MySQL;DB_CLOSE_ON_EXIT=FALSE",
...
class CalendarConversationControllerTest {
  ...
  void
      givenAuthenticatedAccount_whenCreateConversation_thenReturnsOnlyConversationIdWithoutAgentCall()
          throws Exception {
    // when, then
    mockMvc
        .perform(post("/api/ai/calendar/conversations"))
  ...
  void givenValidMessage_whenSendMessage_thenPersistsSuccessfulExchangeAndReturnsMinimalResponse()
    ...
    when(assistantAgent.answer(any()))
        .thenReturn(CalendarAssistantAnswer.withoutBlocks("일정을 확인했어요."));

    // when
    MvcResult result =
        mockMvc
            .perform(
                post("/api/ai/calendar/conversations/{conversationId}/messages", conversationId)
  ...
  void givenAgentAnswerWithMutationPreview_whenSendMessage_thenReturnsMutationPreviewBlock()
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.blocks[0].items[0].after.startAt").value("2026-08-21T06:00:00Z"));
  }

  @Test
  @DisplayName("유효하지 않은 timezone은 message 저장과 agent 호출 전에 거부한다")
  void givenInvalidTimeZone_whenSendMessage_thenRejectsBeforePersistenceOrAgentCall()
      throws Exception {
    // given
    String conversationId = createConversation();
  ...
  void givenConversationOwnedByAnotherAccount_whenSendMessage_thenRejectsWithoutAgentCall()
      throws Exception {
    // given
  ...
  void givenProviderFailure_whenSendMessage_thenKeepsOnlyUserMessage() throws Exception {
    ...
    String conversationId = createConversation();
    when(assistantAgent.answer(any()))
        .thenThrow(new CalioException(ErrorCode.AI_CALENDAR_PROVIDER_UNAVAILABLE));
