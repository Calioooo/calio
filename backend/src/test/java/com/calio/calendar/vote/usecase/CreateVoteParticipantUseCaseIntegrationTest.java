...
class CreateVoteParticipantUseCaseIntegrationTest {
  ...
  void givenConcurrentCaseInsensitiveNicknameRequests_whenCreate_thenCreatesOnlyOneParticipant()
      throws Exception {
    // given
    ...
    try {
      Future<CreateResult> lowerCaseResult =
          executorService.submit(() -> createParticipantAfterStart(ready, start, "calio"));
      Future<CreateResult> upperCaseResult =
          executorService.submit(() -> createParticipantAfterStart(ready, start, "CALIO"));
      ...
      List<CreateResult> results =
          List.of(
              lowerCaseResult.get(5, TimeUnit.SECONDS), upperCaseResult.get(5, TimeUnit.SECONDS));

      // then
      assertThat(results).extracting(CreateResult::created).containsExactlyInAnyOrder(true, false);
      assertThat(results)
          .filteredOn(result -> !result.created())
  ...
  private CreateResult createParticipantAfterStart(
      CountDownLatch ready, CountDownLatch start, String nickname) throws InterruptedException {
    ready.countDown();
    start.await();
    try {
      createVoteParticipantUseCase.createParticipant(voteRoom.getPublicId(), null, nickname, null);
