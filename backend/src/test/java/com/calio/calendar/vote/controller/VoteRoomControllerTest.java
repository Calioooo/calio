...
@SpringBootTest(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:vote-room-controller-test;MODE=MySQL;DB_CLOSE_ON_EXIT=FALSE",
...
class VoteRoomControllerTest {
  ...
  void unauthenticatedRequestCannotCreateOrListVoteRooms() throws Exception {
    ...
    mockMvc
        .perform(get("/api/vote-rooms/me").with(anonymous()))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.errorCode").value("AUTH_TOKEN_REQUIRED"));
  }
  ...
  void givenVoteRoomsFromDifferentCreators_whenListMine_thenReturnsOnlyCurrentAccountsVoteRooms()
      throws Exception {
    // given
