...
class RecurrenceEventApplicationServiceTest {
    ...
    void masterCreateMutatesBeforeEnqueueingSnapshot() {
        ...
        order.verify(enqueueService).enqueueRecurrence(
                org.mockito.ArgumentMatchers.eq(10L), org.mockito.ArgumentMatchers.eq(40L),
                org.mockito.ArgumentMatchers.eq(GoogleCalendarRecurrenceJobKind.RECURRENCE_CREATE), any());
        ArgumentCaptor<GoogleRecurrenceJobPayload> payload = ArgumentCaptor.forClass(
                GoogleRecurrenceJobPayload.class);
        verify(enqueueService).enqueueRecurrence(
                eq(10L), eq(40L), eq(GoogleCalendarRecurrenceJobKind.RECURRENCE_CREATE), payload.capture());
        assertThat(payload.getValue()).isEqualTo(new GoogleRecurrenceJobPayload(
    ...
    void overrideMutationEnqueuesExactOriginSnapshot() {
        ...
        verify(enqueueService, never()).enqueueRecurrence(any(), any(), any(), any());
    }

    @Test
    @DisplayName("master update는 수정된 recurrence snapshot으로 RECURRENCE_UPDATE Job을 enqueue한다")
    void updateRecurrenceEventEnqueuesMasterUpdate() {
        ...
        // then
        verify(enqueueService).enqueueRecurrence(
                eq(10L), eq(40L), eq(GoogleCalendarRecurrenceJobKind.RECURRENCE_UPDATE), any());
    }
    ...
    void deleteRecurrenceEventEnqueuesMasterDelete() {
        ...
        verify(enqueueService).enqueueRecurrenceDeleted(10L, 40L);
    }
