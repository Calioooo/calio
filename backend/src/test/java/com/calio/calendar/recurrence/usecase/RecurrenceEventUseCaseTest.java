...
class RecurrenceEventUseCaseTest {
    ...
    void createPersistsBeforePublishingFinalState() {
        ...
        when(recurrenceEventRepository.saveAndFlush(any())).thenAnswer(invocation -> {
            RecurrenceEvent recurrenceEvent = invocation.getArgument(0);
            ReflectionTestUtils.setField(recurrenceEvent, "id", 40L);
            return recurrenceEvent;
        });
        CreateRecurrenceEventUseCase useCase = new CreateRecurrenceEventUseCase(
                accountRepository,
                tagRepository,
    ...
    void updateOccurrencePersistsAndPublishesExactOrigin() {
        ...
        UpdateRecurrenceOccurrenceUseCase useCase = new UpdateRecurrenceOccurrenceUseCase(
                recurrenceEventRepository,
                overrideRepository,
                recurrenceEngine,
                changePublisher
        ...
        verify(changePublisher).recurrenceOccurrenceUpdated(
                eq(10L),
                org.mockito.ArgumentMatchers.argThat(override ->
