...
class GroupCalendarControllerTest {
  ...
  void givenActiveMember_whenManagingGroupTagAndEvent_thenKeepsGroupScopedApiContract()
      throws Exception {
    // given
    ...
    MvcResult createdTag =
        mockMvc
            .perform(
                post("/api/group-spaces/{groupSpaceId}/tags", groupSpace.getId())
    ...
    mockMvc
        .perform(
            post("/api/group-spaces/{groupSpaceId}/events", groupSpace.getId())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(
            patch("/api/group-spaces/{groupSpaceId}/tags/{tagId}", groupSpace.getId(), tagId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(tagRequest("회의", "#0EA5E9")))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.title").value("회의"));

    MvcResult createdEvent =
        mockMvc
            .perform(
                post("/api/group-spaces/{groupSpaceId}/events", groupSpace.getId())
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(
            get("/api/group-spaces/{groupSpaceId}/events", groupSpace.getId())
    ...
    mockMvc
        .perform(
            get("/api/group-spaces/{groupSpaceId}/calendar/items", groupSpace.getId())
                .param("from", START_AT.minusSeconds(1).toString())
                .param("to", END_AT.plusSeconds(1).toString()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(1)))
    ...
    mockMvc
        .perform(delete("/api/group-spaces/{groupSpaceId}/tags/{tagId}", groupSpace.getId(), tagId))
    ...
    mockMvc
        .perform(
            delete(
                "/api/group-spaces/{groupSpaceId}/events/{eventId}", groupSpace.getId(), eventId))
        .andExpect(status().isNoContent());
  }

  @Test
  @DisplayName("Group Calendar API는 비멤버와 필수값이 누락된 요청을 거절한다")
  void givenNonMemberOrInvalidRequest_whenCallingCalendarApi_thenReturnsContractError()
      throws Exception {
    // given
    ...
    mockMvc
        .perform(
            post("/api/group-spaces/{groupSpaceId}/recurrence-events", groupSpace.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{}"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.title").value("VALIDATION_FAILED"));
  ...
  void givenDirectAndRecurringEvents_whenListCalendarItems_thenSerializesOccurrenceKey()
      throws Exception {
    GroupSpace groupSpace =
        groupSpaceRepository.saveAndFlush(new GroupSpace(currentAccountId(), "group", null));
    groupMemberRepository.saveAndFlush(
  ...
  void givenActiveMember_whenManagingRecurrence_thenKeepsRecurrenceApiContract() throws Exception {
    ...
    mockMvc
        .perform(
            get(
                "/api/group-spaces/{groupSpaceId}/recurrence-events/{recurrenceId}",
                groupSpace.getId(),
                recurrenceId))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            patch(
                    "/api/group-spaces/{groupSpaceId}/recurrence-events/{recurrenceId}/occurrences",
                    groupSpace.getId(),
                    recurrenceId)
                .contentType(MediaType.APPLICATION_JSON)
