...
class SingleEventControllerTest {
  ...
  void
      givenValidEventRequest_whenCreateEvent_thenReturnsPersistedEventWithServerManagedAuditFields()
          throws Exception {
    // given
    String requestBody =
        """
  ...
  void givenUtcMidnightExclusiveRange_whenCreateAllDayEvent_thenReturnsStoredAllDay()
      throws Exception {
    // when, then
    MvcResult result =
        mockMvc
  ...
  void givenMissingOrInvalidTimeZone_whenCreateTimedEvent_thenReturnsInvalidTimeZone()
      throws Exception {
    // when, then
    for (String timeZoneField :
        new String[] {"", ", \"timeZone\": \"\"", ", \"timeZone\": \"Invalid/Zone\""}) {
      mockMvc
          .perform(
              post("/api/events")
                  .contentType(MediaType.APPLICATION_JSON)
  ...
  void givenTimeZone_whenCreateAllDayEvent_thenReturnsInvalidAllDaySchedule() throws Exception {
    // when, then
    mockMvc
        .perform(
            post("/api/events")
  ...
  void givenTimedEvent_whenUpdateToAllDay_thenReplacesCanonicalSchedule() throws Exception {
    // given
    long eventId = createEvent("Timed", "2026-06-13T09:00:00Z", "2026-06-13T10:00:00Z");
  ...
  void givenStartAtIsNotEarlierThanEndAt_whenCreateEvent_thenReturnsInvalidTimeRange()
      throws Exception {
    // given
    String requestBody =
        """
  ...
  void givenMissingImportantEventField_whenUpdateImportantEvent_thenReturnsValidationFailed()
    ...
    // given
    long eventId =
        createEvent("Missing important field", "2026-06-13T00:00:00Z", "2026-06-13T01:00:00Z");

    // when
    mockMvc
        .perform(
            patch("/api/events/{eventId}/important-event", eventId)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{}"))
        // then
        .andExpect(status().isBadRequest())
  ...
  void givenNonBooleanImportantEventField_whenUpdateImportantEvent_thenReturnsValidationFailed()
    ...
    // given
    long eventId =
        createEvent("Invalid important field", "2026-06-14T00:00:00Z", "2026-06-14T01:00:00Z");
  ...
  void givenChangedImportantEvent_whenGetEvent_thenReturnsStoredImportantEventState()
    ...
    updateImportantEventResult(eventId, true).andExpect(status().isOk());
  ...
  void givenExistingEvent_whenUpdateEvent_thenReplacesMutableFieldsAndPreservesServerManagedFields()
    ...
    // given
    MvcResult createResult =
        createEventResult("Original", "2026-06-04T00:00:00Z", "2026-06-04T01:00:00Z");
    JsonNode createdEvent = readResponse(createResult);
    long eventId = createdEvent.get("id").asLong();
    MvcResult persistedResult =
        mockMvc
            .perform(get("/api/events/{eventId}", eventId))
            .andExpect(status().isOk())
            .andReturn();
  ...
  void givenMissingRequiredTimeFields_whenUpdateEvent_thenReturnsValidationFailed()
      throws Exception {
    // given
  ...
  void givenStartAtIsNotEarlierThanEndAt_whenUpdateEvent_thenReturnsInvalidTimeRange()
      throws Exception {
    // given
  ...
  void givenMissingEventId_whenUpdateEvent_thenReturnsEventNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/events/{eventId}", missingEventId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(requestBody))
        // then
        .andExpect(status().isNotFound())
  ...
  void givenMissingEventId_whenGetEvent_thenReturnsEventNotFound() throws Exception {
    ...
    // when
    mockMvc
        .perform(get("/api/events/{eventId}", missingEventId))
        // then
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.title").value("EVENT_NOT_FOUND"))
        .andExpect(jsonPath("$.detail").isString());
  ...
  void givenExistingEvent_whenDeleteEvent_thenReturnsNoContentAndEventIsHiddenFromReads()
      throws Exception {
    // given
    ...
    MvcResult deleteResult =
        mockMvc
            .perform(delete("/api/events/{eventId}", deletedEventId))
            // then
    ...
    mockMvc
        .perform(get("/api/events/{eventId}", deletedEventId))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.title").value("EVENT_NOT_FOUND"));
  ...
  void givenMissingOrAlreadyDeletedEventId_whenDeleteEvent_thenReturnsEventNotFound()
      throws Exception {
    // given
    ...
    // when, then
    mockMvc
        .perform(delete("/api/events/{eventId}", missingEventId))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.title").value("EVENT_NOT_FOUND"))
        .andExpect(jsonPath("$.detail").isString())
        .andExpect(jsonPath("$.*", hasSize(6)));
  ...
  void
      givenChangedImportantEvent_whenListEvents_thenReturnsStoredImportantEventStateSortedByStartAt()
          throws Exception {
    // given
    ...
    updateImportantEventResult(importantId, true).andExpect(status().isOk());
  ...
  private ResultActions updateImportantEventResult(long eventId, boolean importantEvent)
      throws Exception {
  ...
  private void mapAsGoogleEvent(long eventId) {
    ...
    GoogleCalendarIntegration integration =
        googleCalendarIntegrationRepository.saveAndFlush(
            new GoogleCalendarIntegration(event.getAccountId()));
    ...
    googleCalendarEventMappingRepository.saveAndFlush(
        new GoogleCalendarEventMapping(connection, event.getId(), "external-" + eventId, "etag"));
  }
