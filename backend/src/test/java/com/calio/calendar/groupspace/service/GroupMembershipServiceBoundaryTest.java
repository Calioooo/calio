...
class GroupMembershipServiceBoundaryTest {
  ...
  void acceptDelegatesInvitationPersistenceToInvitationServices() {
    ...
    GroupMembershipCommandService membershipCommandService =
        mock(GroupMembershipCommandService.class);
    ...
    GroupInvitationCommandService invitationCommandService =
        mock(GroupInvitationCommandService.class);
    ...
    GroupMember issuer =
        new GroupMember(groupSpace, 1L, "issuer", Instant.parse("2026-08-01T00:00:00Z"));
    ...
    when(invitationQueryService.getInvitationByCredentialHash(
            InvitationCredentialType.LINK_TOKEN, credentialHash))
    ...
    when(invitationCommandService.lockInvitation(
            invitation.getId(), InvitationCredentialType.LINK_TOKEN, credentialHash))
        .thenReturn(invitation);
    ...
    service.accept(
        ...
        new AcceptGroupInvitationRequest(
            GroupInvitationAcceptCredentialType.LINK_TOKEN, "credential", "joined"));
