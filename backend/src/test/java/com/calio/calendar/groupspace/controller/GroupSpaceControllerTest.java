...
class GroupSpaceControllerTest {
  ...
  void detailHidesGroupFromNonMember() throws Exception {
    ...
    GroupSpace groupSpace =
        groupSpaceRepository.saveAndFlush(new GroupSpace(anotherAccount.getId(), "Hidden", null));
    groupMemberRepository.saveAndFlush(
        new GroupMember(groupSpace, anotherAccount.getId(), "owner", Instant.now()));

    // when, then
    mockMvc
        .perform(get("/api/group-spaces/{groupSpaceId}", groupSpace.getId()))
  ...
  void patchUpdatesRequiredNameAndClearsExplicitNullEmoji() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/group-spaces/{groupSpaceId}", groupSpaceId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
            ...
            patch("/api/group-spaces/{groupSpaceId}", groupSpaceId)
                ...
                .content(
                    objectMapper.writeValueAsString(
                        java.util.Map.of("name", "Original", "emoji", opaqueEmoji))))
        // then
        .andExpect(status().isOk())
  ...
  void activeNonOwnerCannotPatch() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/group-spaces/{groupSpaceId}", groupSpace.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  void ownerDeleteHardDeletesGroupData() throws Exception {
    ...
    // when, then
    mockMvc
        .perform(delete("/api/group-spaces/{groupSpaceId}", groupSpaceId))
