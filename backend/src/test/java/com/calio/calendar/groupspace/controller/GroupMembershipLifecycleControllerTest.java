...
class GroupMembershipLifecycleControllerTest {
  ...
  void acceptReturnsCreatedThenOkForExistingActiveMember() throws Exception {
    ...
    // when
    MvcResult joined =
        accept(joinerToken, "joiner")
            // then
            .andExpect(status().isCreated())
            .andExpect(
                header().string("Location", "/api/group-spaces/" + fixture.groupSpace().getId()))
  ...
  void rejoinRejectsActiveNicknameConflict() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/group-spaces/{groupSpaceId}/members/me", fixture.groupSpace().getId())
  ...
  void acceptMemberCountExcludesInactiveMembershipHistory() throws Exception {
    ...
    GroupMember departedMember =
        groupMemberRepository.saveAndFlush(
            new GroupMember(
                fixture.groupSpace(), departedAccount.getId(), "departed", MEMBER_CREATED_AT));
  ...
  void rejoinedGroupSpaceIsListedFirstByStatusChangedAt() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/group-spaces/{groupSpaceId}/members/me", groupA.groupSpace().getId())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + joinerToken))
        .andExpect(status().isNoContent());
    ...
    mockMvc
        .perform(
            get("/api/group-spaces").header(HttpHeaders.AUTHORIZATION, "Bearer " + joinerToken))
        .andExpect(status().isOk())
  ...
  void groupMembershipResponseIncludesIndependentTimestamps() throws Exception {
    ...
    MvcResult result =
        mockMvc
            .perform(get("/api/group-spaces/{groupSpaceId}", fixture.groupSpace().getId()))
            .andExpect(status().isOk())
            ...
            .andReturn();
    JsonNode membership =
        objectMapper.readTree(result.getResponse().getContentAsByteArray()).get("myMembership");
  ...
  void changeAnonymousSharingUpdatesMyGroupMembershipPolicy() throws Exception {
    ...
    mockMvc
        .perform(
            patch(
                    "/api/group-spaces/{groupSpaceId}/members/me/anonymous-sharing",
    ...
    assertThat(
            groupMemberRepository
                .findById(fixture.ownerMember().getId())
                .orElseThrow()
                .isAnonymous())
        .isTrue();
  }
  ...
  void memberListConcealsGroupFromNonMember() throws Exception {
    ...
    GroupSpace hiddenGroup =
        groupSpaceRepository.saveAndFlush(new GroupSpace(anotherOwner.getId(), "Hidden", null));
    groupMemberRepository.saveAndFlush(
        new GroupMember(hiddenGroup, anotherOwner.getId(), "owner", MEMBER_CREATED_AT));

    // when, then
    mockMvc
        .perform(get("/api/group-spaces/{groupSpaceId}/members", hiddenGroup.getId()))
  ...
  void acceptValidationFailureUsesProblemErrorCode() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/group-invitations/accept")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  private org.springframework.test.web.servlet.ResultActions accept(
      String accessToken, String nickname) throws Exception {
    return accept(accessToken, LINK_TOKEN, nickname);
  }

  private org.springframework.test.web.servlet.ResultActions accept(
      String accessToken, String linkToken, String nickname) throws Exception {
  ...
  private GroupFixture createGroupFixture() {
    ...
    GroupSpace groupSpace =
        groupSpaceRepository.saveAndFlush(new GroupSpace(ownerAccountId, "Shared", null));
  ...
  private void createInvitation(GroupSpace groupSpace, GroupMember ownerMember, String linkToken) {
    groupInvitationRepository.saveAndFlush(
        new GroupInvitation(
            groupSpace.getId(),
            ownerMember.getId(),
