...
class GroupMemberRepositoryTest {
  ...
  void listMembershipsLoadsGroupSpaceAndPreservesTieBreakOrder() {
    ...
    GroupSpace secondGroup = groupSpaceRepository.saveAndFlush(new GroupSpace(1L, "second", null));
    groupMemberRepository.saveAllAndFlush(
        List.of(
  ...
  void getActiveMembershipLoadsGroupSpace() {
    ...
    groupMemberRepository.saveAndFlush(
        new GroupMember(groupSpace, 1L, "owner", Instant.parse("2026-08-12T00:00:00Z")));
    entityManager.clear();
    ...
    GroupMember membership =
        groupMemberRepository
            .findByGroupSpaceIdAndAccountIdAndStatus(
                groupSpace.getId(), 1L, GroupMemberStatus.ACTIVE)
            .orElseThrow();
