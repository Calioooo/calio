...
@SpringBootTest(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:group-invitation-controller-test;MODE=MySQL;DB_CLOSE_ON_EXIT=FALSE",
...
class GroupInvitationControllerTest {
  ...
  @Autowired private GroupInvitationService invitationService;
  ...
  void invitationLifecycleContract() throws Exception {
    ...
    MvcResult issueResult =
        mockMvc
            .perform(post("/api/group-spaces/{groupSpaceId}/invitations", groupSpaceId))
            // then
            .andExpect(status().isCreated())
            .andExpect(jsonPath("$.invitationId").isNumber())
            .andExpect(
                jsonPath("$.inviteUrl")
                    .value(org.hamcrest.Matchers.startsWith("https://calio.app/invite/")))
            .andExpect(
                jsonPath("$.inviteCode")
                    .value(
    ...
    assertThat(issueResult.getResponse().getHeader("Location"))
        .isEqualTo("/api/group-spaces/%d/invitations/%d".formatted(groupSpaceId, invitationId));

    mockMvc
        .perform(get("/api/group-spaces/{groupSpaceId}/invitations", groupSpaceId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.invitations", hasSize(1)))
        .andExpect(jsonPath("$.invitations[0].invitationId").value(invitationId))
        .andExpect(jsonPath("$.invitations[0].expiresAt").value("2026-07-29T08:00:00Z"))
    ...
    mockMvc
        .perform(
            post("/api/group-invitations/preview")
    ...
    GroupMember issuer =
        memberRepository
            .findByGroupSpaceIdAndAccountIdAndStatus(
                groupSpaceId, currentAccountId(), GroupMemberStatus.ACTIVE)
            .orElseThrow();
