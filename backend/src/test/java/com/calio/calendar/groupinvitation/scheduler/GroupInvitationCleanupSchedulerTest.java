...
class GroupInvitationCleanupSchedulerTest {
  ...
  @Mock private GroupInvitationService groupInvitationService;
  ...
  void repeatsFullBatchesUntilPartialBatch() {
    ...
    GroupInvitationCleanupScheduler scheduler =
        new GroupInvitationCleanupScheduler(
            groupInvitationService, properties, Clock.fixed(now, ZoneOffset.UTC));
  ...
  void logsCleanupFailureWithoutPropagating(CapturedOutput output) {
    ...
    GroupInvitationCleanupScheduler scheduler =
        new GroupInvitationCleanupScheduler(
            groupInvitationService, properties(2, 3), Clock.fixed(now, ZoneOffset.UTC));
