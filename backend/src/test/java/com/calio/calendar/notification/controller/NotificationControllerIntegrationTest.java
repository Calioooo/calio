...
class NotificationControllerIntegrationTest {
  ...
  @Autowired private IosPushDeviceRepository endpointRepository;
  ...
  void givenRegisteredEndpoint_whenDeactivate_thenRemovesEligibleEndpoint() throws Exception {
    mockMvc
        .perform(
            put("/api/notification-endpoints/ios")
    ...
    Long pushDeviceId = endpointRepository.findByApnsToken("device-token").orElseThrow().getId();
    ...
    mockMvc
        .perform(delete("/api/notification-endpoints/ios/{installationId}", "iphone-installation"))
        .andExpect(status().isNoContent());
  ...
  void givenTokenOwnedByAnotherInstallation_whenRegister_thenTransfersTokenOwnership()
    ...
    Long previousEndpointId =
        endpointRepository.findByApnsToken("transferred-device-token").orElseThrow().getId();
