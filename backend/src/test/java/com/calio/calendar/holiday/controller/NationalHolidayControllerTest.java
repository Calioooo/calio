...
class NationalHolidayControllerTest {
  ...
  void
      givenNationalHolidaysAcrossRange_whenListNationalHolidays_thenReturnsInclusiveRangeSortedByDateAndTitle()
          throws Exception {
    // given
    nationalHolidayRepository.save(new NationalHoliday(LocalDate.of(2026, 5, 4), "Before"));
    NationalHoliday childrensDay =
        nationalHolidayRepository.save(
            new NationalHoliday(LocalDate.of(2026, 5, 5), "Children's Day"));
    NationalHoliday alternateHoliday =
        nationalHolidayRepository.save(
            new NationalHoliday(LocalDate.of(2026, 5, 5), "Alternate Holiday"));
    NationalHoliday memorialDay =
        nationalHolidayRepository.save(
    ...
    mockMvc
        .perform(
            get("/api/national-holidays").param("from", "2026-05-05").param("to", "2026-06-06"))
        // then
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(3)))
        .andExpect(
            jsonPath("$[0].nationalHolidayId").value(alternateHoliday.getNationalHolidayId()))
  ...
  void givenSameFromAndTo_whenListNationalHolidays_thenReturnsOnlyThatDateHolidays()
      throws Exception {
    // given
    nationalHolidayRepository.save(new NationalHoliday(LocalDate.of(2026, 5, 4), "Before"));
    NationalHoliday childrensDay =
    ...
    mockMvc
        .perform(
            get("/api/national-holidays").param("from", "2026-05-05").param("to", "2026-05-05"))
        // then
        .andExpect(status().isOk())
  ...
  void givenFromAfterTo_whenListNationalHolidays_thenReturnsInvalidTimeRange() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/national-holidays").param("from", "2026-06-06").param("to", "2026-05-05"))
        // then
  ...
  void givenMissingDateParameter_whenListNationalHolidays_thenReturnsValidationFailed()
      throws Exception {
    // when, then
    mockMvc
        .perform(get("/api/national-holidays").param("to", "2026-05-05"))
        .andExpect(status().isBadRequest())
    ...
    mockMvc
        .perform(get("/api/national-holidays").param("from", "2026-05-05"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.title").value("VALIDATION_FAILED"))
        .andExpect(jsonPath("$.detail").value("Validation failed."))
        .andExpect(jsonPath("$.*", hasSize(6)));
  ...
  void givenInvalidDateFormat_whenListNationalHolidays_thenReturnsValidationFailed()
      throws Exception {
    // when
    mockMvc
        .perform(
  ...
  void givenNationalHolidayAndEvent_whenListEvents_thenReturnsOnlyEventResponseShape()
      throws Exception {
    // given
