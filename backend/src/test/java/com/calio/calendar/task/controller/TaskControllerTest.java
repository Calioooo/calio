...
class TaskControllerTest {
  ...
  void givenValidTaskTitle_whenCreateTask_thenReturnsPersistedTaskWithServerManagedDefaults()
    ...
    MvcResult result =
        mockMvc
            .perform(
                post("/api/tasks").contentType(MediaType.APPLICATION_JSON).content(requestBody))
            // then
  ...
  void givenCreatedTasks_whenListTasks_thenReturnsActiveTasksSortedByTaskIdAscending()
      throws Exception {
    // given
  ...
  void givenActiveTask_whenUpdateTaskTitle_thenReturnsUpdatedTaskAndPersistsOnlyTitle()
      throws Exception {
    // given
  ...
  void givenCompletedTask_whenUpdateTaskTitle_thenReturnsCompletedTaskTitleUpdateNotAllowed()
      throws Exception {
    // given
    ...
    mockMvc
        .perform(
            patch("/api/tasks/{taskId}", task.getTaskId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  void givenMissingTaskId_whenUpdateTaskTitle_thenReturnsTaskNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/tasks/{taskId}", 999999L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  void givenMissingBody_whenUpdateTaskTitle_thenReturnsValidationFailed() throws Exception {
    ...
    // when
    mockMvc
        .perform(patch("/api/tasks/{taskId}", taskId).contentType(MediaType.APPLICATION_JSON))
        // then
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.title").value("VALIDATION_FAILED"))
        .andExpect(jsonPath("$.detail").value("Validation failed."))
  ...
  void givenInvalidTaskIdPathVariable_whenUpdateTaskTitle_thenReturnsValidationFailed()
      throws Exception {
    // when
    mockMvc
        .perform(
  ...
  void givenActiveTask_whenCompleteTask_thenReturnsCompletedTaskAndPersistsCompletedAt()
      throws Exception {
    // given
    ...
    MvcResult result =
        mockMvc
            .perform(delete("/api/tasks/{taskId}", taskId))
            // then
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.taskId").value(taskId))
    ...
    assertThat(task.getCompletedAt())
        .isEqualTo(Instant.parse(response.get("completedAt").asString()));
  }
  ...
  void givenCompletedTask_whenCompleteTaskAgain_thenPreservesCompletedAt() throws Exception {
    ...
    // when
    mockMvc
        .perform(delete("/api/tasks/{taskId}", task.getTaskId()))
        // then
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.isCompleted").value(true))
  ...
  void givenCompletedTask_whenUncompleteTask_thenReturnsActiveTaskAndClearsCompletedAt()
      throws Exception {
    // given
    ...
    // when
    mockMvc
        .perform(patch("/api/tasks/{taskId}/uncomplete", task.getTaskId()))
        // then
        .andExpect(status().isOk())
  ...
  void givenActiveTask_whenUncompleteTask_thenKeepsActiveState() throws Exception {
    ...
    // when
    mockMvc
        .perform(patch("/api/tasks/{taskId}/uncomplete", taskId))
        // then
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.taskId").value(taskId))
        .andExpect(jsonPath("$.isCompleted").value(false))
  ...
  void givenCompletedTask_whenListTasks_thenExcludesUntilUncompleted() throws Exception {
    ...
    long firstTaskId = createTask("First task");
    long secondTaskId = createTask("Second task");
    mockMvc.perform(delete("/api/tasks/{taskId}", firstTaskId)).andExpect(status().isOk());
    ...
    // when
    mockMvc
        .perform(patch("/api/tasks/{taskId}/uncomplete", firstTaskId))
        .andExpect(status().isOk());
  ...
  void givenMissingTaskId_whenCompleteTask_thenReturnsTaskNotFound() throws Exception {
    // when
    mockMvc
        .perform(delete("/api/tasks/{taskId}", 999999L))
        // then
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.title").value("TASK_NOT_FOUND"))
        .andExpect(jsonPath("$.detail").value("Task not found."));
  ...
  void givenMissingTaskId_whenUncompleteTask_thenReturnsTaskNotFound() throws Exception {
    // when
    mockMvc
        .perform(patch("/api/tasks/{taskId}/uncomplete", 999999L))
        // then
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.title").value("TASK_NOT_FOUND"))
        .andExpect(jsonPath("$.detail").value("Task not found."));
