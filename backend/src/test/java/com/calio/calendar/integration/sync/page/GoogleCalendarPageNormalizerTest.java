...
class GoogleCalendarPageNormalizerTest {
  ...
  void setUp() {
    normalizer =
        new GoogleCalendarPageNormalizer(
            eventMappingQueryService,
            recurrenceMappingQueryService,
            timeNormalizer,
            recurrenceMapper,
            recurrenceEventLoader);
    when(eventMappingQueryService.listEventMappings(any(), any(), any())).thenReturn(List.of());
  ...
  void givenOverrideBeforeRecurrenceEvent_whenNormalize_thenOrdersRecurrenceEventFirst() {
    ...
    GoogleCalendarNormalizedPage page =
        normalizer.normalize(
            10L, page(deletedOccurrence), new GoogleCalendarSyncRunContext("token"));

    // then
    assertThat(page.items()).containsExactly(recurrenceEvent, override);
  }
  ...
  private GoogleCalendarEventResponse cancelledItem(String id) {
    return new GoogleCalendarEventResponse(
        id, "cancelled", null, null, null, null, List.of(), null, null, null, null);
  }
