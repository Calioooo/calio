...
class GoogleCalendarEventJobServiceTest {
  ...
  void setUp() {
    ...
    service =
        new GoogleCalendarEventJobService(
  ...
  void givenDisconnectedMapping_whenApplyUpdate_thenRecordsLocalChangeOnly() {
    ...
    GoogleCalendarEventMapping mapping =
        new GoogleCalendarEventMapping(connection, 40L, "external-1", "etag-1");
    GoogleCalendarEventJob job = job(GoogleCalendarEventJobKind.UPDATE);
    when(mappingQueryService.listEventMappingsForEvent(20L, 40L)).thenReturn(List.of(mapping));
  ...
  void givenProviderConflict_whenApplyCreate_thenDoesNotInsertGoogleEvent() {
    ...
    verify(mappingCommandService, never())
        .createEventMapping(org.mockito.ArgumentMatchers.any(GoogleCalendarEventMapping.class));
    verify(jobService).completeWithConflict(50L, 10L, "worker");
  }
  ...
  void givenEventDeletedAfterCreateJobEnqueued_whenExecute_thenCreatesGoogleEventFromSnapshot() {
    ...
    verify(eventsClient).post(org.mockito.ArgumentMatchers.eq("token"), requestCaptor.capture());
