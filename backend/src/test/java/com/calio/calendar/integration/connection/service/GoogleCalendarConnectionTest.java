...
class GoogleCalendarConnectionTest {
  ...
  void givenConnectedProviderData_whenReconnect_thenReplacesIdentityAndInvalidatesOldRun() {
    ...
    SingleEvent importedEvent =
        createMappedEvent(account, integration, "external-before-reconnect");
  ...
  void givenConnectedProviderData_whenDeleteIntegration_thenDeletesProviderDataFirst() {
    ...
    GoogleCalendarIntegration integration =
        integrationRepository.saveAndFlush(integration(account.getId()));
    SingleEvent importedEvent =
        createMappedEvent(account, integration, "external-before-disconnect");
  ...
  @Test
  @DisplayName(
      "disconnect시 recurrence event의 override를 먼저 삭제하고 (recurrence event도 삭제), local recurrence는 삭제하지 않는다")
  ...
  private void stubGoogleConnection() {
    ...
    when(googleOAuthClient.fetchUserInfo("new-access-token"))
        .thenReturn(new GoogleUserInfoResponse("new-google-subject", "new-user@example.com"));
  ...
  private SingleEvent createMappedEvent(
    ...
    mappingRepository.saveAndFlush(
        new GoogleCalendarEventMapping(integration, event, externalEventId, null, null));
    return event;
  }
