...
class GoogleOperationJobQueryServiceTest {
  ...
  private final GoogleOperationJobQueryService service =
      new GoogleOperationJobQueryService(repository);

  @Test
  @DisplayName(
      "recurrence-event inbound cleanup은 같은 recurrence aggregate의 pending outbound Job을 보호한다")
  void recurrenceEventScopeQueriesAggregateJobs() {
    ...
    assertThat(
            service.hasPendingOutboundJob(
                10L, 20L, GoogleCalendarEffectiveScope.recurrenceEvent(40L)))
        .isTrue();
  ...
  void eventScopeQueriesEventJobs() {
    ...
    // when
    boolean hasPendingJob =
        service.hasPendingOutboundJob(10L, 20L, GoogleCalendarEffectiveScope.event(40L));
