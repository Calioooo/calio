...
class GoogleCalendarIntegrationDataServiceTest {
  ...
  void givenUnseenMappings_whenFinalizeFullSync_thenDeletesByBatchAndRenewsLease() {
    ...
    when(recurrenceMappingQueryService.listOverrideMappingBatch(1L, 0L, 500)).thenReturn(List.of());
    when(eventMappingQueryService.listEventMappingBatch(1L, 0L, 500))
        .thenReturn(List.of(eventMapping));
    when(eventMappingQueryService.listEventMappingBatch(1L, 10L, 500)).thenReturn(List.of());
    ...
    verify(singleEventRepository).deleteAllByIdInBatch(List.of(20L));
  ...
  @Test
  @DisplayName(
      "같은 recurrence event를 참조하는 mapping이 남아 있으면 FULL SYNC cleanup은 recurrence aggregate를 삭제하지 않는다")
  void
      givenUnseenRecurrenceMappingWithRemainingRecurrenceReference_whenFinalizeFullSync_thenKeepsAggregate() {
    GoogleCalendarConnection connection = mock(GoogleCalendarConnection.class);
    GoogleCalendarIntegration integration = mock(GoogleCalendarIntegration.class);
  ...
  @Test
  @DisplayName(
      "다른 connection override mapping이 남아 있으면 FULL SYNC cleanup은 canonical override를 삭제하지 않는다")
  void
      givenUnseenOverrideWithAnotherConnectionMapping_whenFinalizeFullSync_thenKeepsCanonicalOverride() {
    GoogleCalendarConnection connection = mock(GoogleCalendarConnection.class);
    GoogleCalendarIntegration integration = mock(GoogleCalendarIntegration.class);
