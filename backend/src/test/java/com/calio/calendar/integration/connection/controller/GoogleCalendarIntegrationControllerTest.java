...
@SpringBootTest(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:calendar-google-integration-controller-test;MODE=MySQL;DB_CLOSE_ON_EXIT=FALSE",
...
class GoogleCalendarIntegrationControllerTest {
  ...
  @Autowired private FakeGoogleOAuthClient googleOAuthClient;

  @Autowired private FakeGoogleCalendarEventsClient googleCalendarEventsClient;
  ...
  void givenUnauthenticatedRequest_whenGetConnection_thenReturnsUnauthorized() throws Exception {
    // when, then
    mockMvc
        .perform(get("/api/integrations/google-calendar").with(anonymous()))
  ...
  void givenNoIntegration_whenGetConnection_thenReturnsDisconnectedState() throws Exception {
    // when, then
    mockMvc
        .perform(get("/api/integrations/google-calendar"))
  ...
  void givenNoIntegration_whenDisconnect_thenReturnsNoContent() throws Exception {
    // when, then
    mockMvc.perform(delete("/api/integrations/google-calendar")).andExpect(status().isNoContent());
  }
  ...
  void givenNoIntegration_whenSync_thenReturnsNotConnected() throws Exception {
    // when, then
    mockMvc
        .perform(post("/api/integrations/google-calendar/sync"))
  ...
  static class GoogleOAuthClientMockConfig {
    ...
    FakeGoogleCalendarEventsClient googleCalendarEventsClientMock(
        GoogleOAuthProperties properties, ObjectMapper objectMapper) {
  ...
  static class FakeGoogleCalendarEventsClient extends GoogleCalendarEventsClient {
    ...
    FakeGoogleCalendarEventsClient(GoogleOAuthProperties properties, ObjectMapper objectMapper) {
