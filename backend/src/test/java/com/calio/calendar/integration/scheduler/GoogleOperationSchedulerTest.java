...
class GoogleOperationSchedulerTest {
  ...
  private final GoogleOperationScheduler scheduler =
      new GoogleOperationScheduler(
  ...
  void givenConnectedAccounts_whenRecoveringAndEnqueuing_thenProcessesKeysetBatches() {
    ...
    when(integrationQueryService.listConnectedAccountIds(500L, 500)).thenReturn(List.of(501L));
