...
@SpringBootTest(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:security-authentication-integration-test;MODE=MySQL;DB_CLOSE_ON_EXIT=FALSE",
...
class SecurityAuthenticationIntegrationTest {
  ...
  void
      givenInvalidBearerToken_whenRequestWithAuthorizationHeader_thenReturnsInvalidTokenProblemDetail()
          throws Exception {
    // when
    mockMvc
        .perform(
            get("/api/security-test/authenticated-account")
  ...
  void
      givenRevokedBearerToken_whenRequestWithAuthorizationHeader_thenReturnsRevokedTokenProblemDetail()
          throws Exception {
    // given
  ...
  void
      givenValidBearerToken_whenRequestWithAuthorizationHeader_thenControllerReceivesAccountIdPrincipal()
          throws Exception {
    // given
    ...
    mockMvc
        .perform(
            get("/api/tasks/security-test/authenticated-account")
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + rawToken))
        // then
  ...
  void givenNoAuthorizationHeader_whenRequestPublicApis_thenDoesNotRequireAuthentication()
    ...
    // when, then
    mockMvc
        .perform(get("/api/national-holidays"))
