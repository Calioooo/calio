...
public class VoteRoomService {
  ...
  public VoteRoomResponse create(Long accountId, CreateVoteRoomRequest request) {
    ...
    if (request.candidateEndDate().isBefore(start)
    ...
    VoteRoom voteRoom =
        voteRoomCommandService.create(
            new VoteRoom(
                UUID.randomUUID(), request.name(), start, request.candidateEndDate(), account));
    return VoteRoomResponse.from(voteRoom);
  }

  public List<VoteRoomResponse> listMine(Long accountId) {
    return voteRoomQueryService.listByCreatedByAccountId(accountId).stream()
