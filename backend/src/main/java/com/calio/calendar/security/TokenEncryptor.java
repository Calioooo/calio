...
public class TokenEncryptor {
  ...
  public TokenEncryptor(@Qualifier("googleTokenBytesEncryptor") BytesEncryptor bytesEncryptor) {
  ...
  private record TokenEnvelope(
      String keyVersion, byte[] nonce, byte[] ciphertext, byte[] authenticationTag) {
    ...
    static TokenEnvelope fromEncryptedBytes(String keyVersion, byte[] encrypted) {
      ...
      byte[] authenticationTag =
          Arrays.copyOfRange(encrypted, authenticationTagOffset, encrypted.length);
    ...
    static TokenEnvelope parse(String value) {
      ...
      if (parts.length != 5
          ...
          || !KEY_VERSION.equals(parts[1])) {
        throw new IllegalArgumentException("Unsupported encrypted token envelope");
      }

      return new TokenEnvelope(
          parts[1], DECODER.decode(parts[2]), DECODER.decode(parts[3]), DECODER.decode(parts[4]));
    }
