...
public class Ical4jRecurrenceEngine implements Rfc5545RecurrenceEngine {
  ...
  private static final String CONTENT_LINE_CALENDAR =
      "BEGIN:VCALENDAR\r\n"
          + "VERSION:2.0\r\n"
  ...
  public List<RecurrenceOccurrence> expand(
    ...
    addRuleCandidates(
        schedule, definition.inclusionRules(), window, includedStarts, candidateLimit);
    addDateCandidates(schedule, definition.inclusionDates(), includedStarts, candidateLimit);

    Set<Instant> excludedStarts = new HashSet<>();
    addRuleCandidates(
        schedule, definition.exclusionRules(), window, excludedStarts, candidateLimit);
  ...
  private void addRuleCandidates(
    ...
    for (ContentLine rule : rules) {
      ...
      List<Temporal> candidates =
          recurrenceRuleForGeneration(schedule, rule)
              .getDatesAsStream(
  ...
  private Optional<RecurrenceOccurrence> toOccurrence(
      RecurrenceSchedule schedule, Instant startAt) {
    if (startAt.equals(schedule.firstOccurrenceStartAt())) {
      return Optional.of(
          new RecurrenceOccurrence(startAt, startAt, schedule.firstOccurrenceEndAt()));
    }
  ...
  private Optional<Instant> resolveTimedOccurrenceEnd(
      RecurrenceSchedule schedule, Instant startAt) {
    ZoneId zoneId = schedule.zoneId();
    LocalDateTime localEnd =
        startAt.atZone(zoneId).toLocalDateTime().plus(schedule.timedWallClockDuration());
    ...
    ZoneOffset endOffset =
        offsets.stream().filter(startOffset::equals).findFirst().orElse(offsets.getFirst());
  ...
  private ExpansionWindow expansionWindow(RecurrenceSchedule schedule, Instant from, Instant to) {
    if (schedule.allDay()) {
      LocalDate requestedFrom =
          from.atOffset(ZoneOffset.UTC).toLocalDate().minusDays(schedule.allDayDurationDays() + 1);
      LocalDate requestedTo = to.atOffset(ZoneOffset.UTC).toLocalDate().plusDays(2);
      return new ExpansionWindow(
          laterDate(requestedFrom, schedule.firstOccurrenceDate()), requestedTo);
    }
    LocalDateTime requestedFrom =
        from.atZone(schedule.zoneId())
            .toLocalDateTime()
            .minus(schedule.timedWallClockDuration())
            .minusDays(1);
    LocalDateTime requestedTo = to.atZone(schedule.zoneId()).toLocalDateTime().plusDays(1);
    return new ExpansionWindow(
        laterDateTime(requestedFrom, schedule.firstOccurrenceLocalStart()), requestedTo);
  }
  ...
  private ExpansionWindow boundedByUntil(
      RecurrenceSchedule schedule, ContentLine line, ExpansionWindow window) {
  ...
  private Recur<Temporal> recurrenceRuleForGeneration(
      RecurrenceSchedule schedule, ContentLine line) {
    Recur<Temporal> rule = recurrenceRule(line);
    if (schedule.allDay() || rule.getUntil() == null) {
      return rule;
    }
    String valueWithoutUntil =
        Stream.of(rule.toString().split(";"))
  ...
  private boolean startsOnOrBeforeUntil(
      RecurrenceSchedule schedule, ContentLine line, Temporal candidate) {
    Temporal until = recurrenceRule(line).getUntil();
    if (until == null) {
      return true;
    }
  ...
  private RecurrenceDefinition parseDefinition(
      RecurrenceSchedule schedule, List<String> recurrenceRules) {
    if (recurrenceRules == null) {
      throw new CalioException(ErrorCode.INVALID_RECURRENCE_RULE);
    }
    List<ContentLine> rules =
        recurrenceRules.stream()
            .map(this::parseContentLine)
            .map(line -> validateLine(schedule, line))
            .distinct()
            .sorted(
                Comparator.comparingInt(this::propertyOrder).thenComparing(ContentLine::normalized))
            .toList();
  ...
  private List<ContentLine> propertiesOfType(List<ContentLine> lines, Class<?> propertyType) {
    return lines.stream().filter(line -> propertyType.isInstance(line.property())).toList();
  }
  ...
  private void validateFirstOccurrence(
      RecurrenceSchedule schedule, RecurrenceDefinition definition) {
    boolean hasEarlierInclusionDate =
        definition.inclusionDates().stream()
            .flatMap(line -> dateValues(line).stream())
            .map(candidate -> occurrenceStart(schedule, candidate))
            .flatMap(Optional::stream)
            .anyMatch(candidate -> candidate.isBefore(schedule.firstOccurrenceStartAt()));
    boolean excludedByDate =
        definition.exclusionDates().stream()
            .flatMap(line -> dateValues(line).stream())
            .map(candidate -> occurrenceStart(schedule, candidate))
            .flatMap(Optional::stream)
            .anyMatch(schedule.firstOccurrenceStartAt()::equals);
    boolean excludedByRule =
        definition.exclusionRules().stream()
  ...
  private ContentLine parseContentLine(String rawLine) {
    ...
    try {
      Calendar calendar =
          new CalendarBuilder()
              .build(new StringReader(CONTENT_LINE_CALENDAR.formatted(rawLine.trim())));
  ...
  private Property recurrenceProperty(Calendar calendar) {
    ...
    List<Property> properties =
        event.getPropertyList().getAll().stream()
  ...
  private void validateRulePartCount(String rawLine, Property property) {
    ...
    long parsedPartCount =
        property.getValue().chars().filter(character -> character == '=').count();
  ...
  private void validateRuleValueType(RecurrenceSchedule schedule, ContentLine line) {
    ...
    boolean hasTimedPart =
        !recur.getHourList().isEmpty()
  ...
  private void validateTimedDates(
      RecurrenceSchedule schedule, Value valueType, TzId timeZone, List<Temporal> dates) {
    boolean hasInvalidType = valueType != null && !Value.DATE_TIME.equals(valueType);
    boolean hasInvalidTimeZone =
        timeZone != null && !timeZone.getValue().equals(schedule.timeZone());
    boolean hasInvalidDate =
        dates.stream().anyMatch(date -> !TemporalAdapter.isDateTimePrecision(date));
    boolean hasGapDate =
        dates.stream()
  ...
  private List<Temporal> dateValues(ContentLine line) {
    ...
    try {
      return dateProperty.getDates().stream().map(Temporal.class::cast).toList();
    } catch (RuntimeException exception) {
      throw new CalioException(ErrorCode.INVALID_RECURRENCE_RULE, exception);
    }
  }
  ...
  private String normalize(Property property) {
    String normalizedParameters =
        property.getParameterList().getAll().stream()
