...
public record RecurrenceSchedule(
    Instant firstOccurrenceStartAt, Instant firstOccurrenceEndAt, boolean allDay, String timeZone) {
  ...
  private static RecurrenceSchedule createAllDay(
      Instant firstOccurrenceStartAt, Instant firstOccurrenceEndAt, String timeZone) {
    boolean usesUtcMidnight =
        isUtcMidnight(firstOccurrenceStartAt) && isUtcMidnight(firstOccurrenceEndAt);
    if (timeZone != null || !usesUtcMidnight) {
      throw new CalioException(ErrorCode.INVALID_RECURRENCE_SCHEDULE);
    }
    return new RecurrenceSchedule(firstOccurrenceStartAt, firstOccurrenceEndAt, true, null);
  }
  ...
  private static RecurrenceSchedule createTimed(
    ...
    return new RecurrenceSchedule(firstOccurrenceStartAt, firstOccurrenceEndAt, false, timeZone);
  }
  ...
  private static void validateCanonicalOffset(
      LocalDateTime localDateTime, Instant instant, ZoneId zoneId) {
  ...
  private static void validateCanonicalEndOffset(
      LocalDateTime localEnd, Instant endAt, Instant startAt, ZoneId zoneId) {
