...
public class TagQueryService {
  ...
  public List<Tag> listAvailableTags(Long accountId) {
    return Stream.concat(
            listGlobalDefaultTags().stream(), listAccountCustomTags(accountId).stream())
  ...
  public boolean hasGroupCustomTagTitle(Long groupSpaceId, String title, Long excludedTagId) {
    if (excludedTagId == null) {
      return tagRepository.existsByTagTypeAndTitleAndGroupSpace_Id(
  ...
  private List<Tag> listGlobalDefaultTags() {
    return tagRepository.findByTagTypeAndAccountIsNullAndGroupSpaceIsNullOrderByIdAsc(
  ...
  private Optional<Tag> getGlobalDefaultTagIfExists(Long tagId) {
    return tagRepository.findByIdAndTagTypeAndAccountIsNullAndGroupSpaceIsNull(
        tagId, TagType.PERSONAL_DEFAULT);
  }
