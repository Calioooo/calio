...
public class GroupTagService {
  ...
  public List<TagResponse> list(Long accountId, Long groupSpaceId) {
    ...
    return tagQueryService.listGroupTags(groupSpaceId).stream().map(TagResponse::from).toList();
  }
  ...
  public TagResponse create(Long accountId, Long groupSpaceId, CustomTagRequest request) {
    ...
    Tag tag =
        tagCommandService.createGroupCustomTag(groupSpace, request.title(), request.colorCode());
    return TagResponse.from(tag);
  }

  @Transactional
  public TagResponse update(
      Long accountId, Long groupSpaceId, Long tagId, CustomTagRequest request) {
    membershipQueryService.getActiveMembership(groupSpaceId, accountId);
