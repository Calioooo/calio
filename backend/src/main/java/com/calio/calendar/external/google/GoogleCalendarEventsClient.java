...
public class GoogleCalendarEventsClient {
  ...
  private static final String EVENT_FIELDS =
      "id,status,etag,updated,summary,description,"
  ...
  private static final String PARTIAL_FIELDS =
      "nextPageToken,nextSyncToken,timeZone," + "items(" + EVENT_FIELDS + ")";
  ...
  public GoogleCalendarEventPage listEvents(
      String accessToken, GoogleCalendarSyncMode mode, String syncToken, String pageToken) {
  ...
  public Optional<GoogleCalendarEventResponse> getEvent(
      String accessToken, String externalEventId) {
  ...
  public GoogleCalendarEventResponse insertEvent(
      String accessToken, String providerIdentity, GoogleEventJobPayload payload) {
    ...
    GoogleCalendarEventWriteRequest request =
        GoogleCalendarEventWriteRequest.forCreate(payload, providerIdentity);
    try {
      ...
      GoogleCalendarEventResponse existing =
          getEvent(accessToken, providerIdentity).orElseThrow(() -> syncFailed(exception));
  ...
  private GoogleCalendarEventResponse requireRequestedEventId(
      GoogleCalendarEventResponse response, String providerIdentity) {
  ...
  private GoogleCalendarEventPage requestPage(
      String accessToken, GoogleCalendarSyncMode mode, String syncToken, String pageToken) {
    GoogleCalendarEventPage response =
        restClient
            .get()
  ...
  private URI eventsUri(GoogleCalendarSyncMode mode, String syncToken, String pageToken) {
    UriComponentsBuilder builder =
        UriComponentsBuilder.fromUriString(properties.getCalendarEventsUrl())
  ...
  private URI existingEventWriteUri(String externalEventId) {
    return eventWriteUriBuilder().pathSegment(externalEventId).build().encode().toUri();
  }
  ...
  private boolean isScopeFailure(RestClientResponseException exception) {
    ...
    try {
      var errors =
          objectMapper.readTree(exception.getResponseBodyAsString()).get("error").get("errors");
  ...
  private void validateQueryContract(
      GoogleCalendarSyncMode mode, String syncToken, String pageToken) {
  ...
  private void logFailure(ErrorCode errorCode, Integer status, Exception exception) {
    log.warn(
