...
public class FindAvailableTimesUseCase {
    ...
    public List<CalendarFreeTime> find(
        ...
        for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
            addAvailableTimesForDate(events, date, timeZone, availableFrom, availableUntil, minimumDuration,
                    availableTimes);
        }
    ...
    private List<EventResponse> listEvents(Long accountId, Instant from, Instant to) {
        ...
        Map<Long, Tag> tagsById = tagRepository.findAllById(events.stream()
                        .map(SingleEvent::getTagId).collect(Collectors.toSet()))
    ...
    private void validateAvailabilityCriteria(
        ...
        if (!availableFrom.isBefore(availableUntil) || minimumDuration.isZero() || minimumDuration.isNegative()) {
    ...
    private void addAvailableTimesForDate(
            List<EventResponse> events, LocalDate date, ZoneId timeZone, LocalTime availableFrom,
            LocalTime availableUntil, Duration minimumDuration, List<CalendarFreeTime> availableTimes
    ) {
        ...
        List<TimeRange> occupiedRanges = events.stream()
                ...
                .toList();
        List<String> allDayTitles = events.stream()
        ...
        for (TimeRange occupiedRange : occupiedRanges) {
            addAvailableTime(availableStart, occupiedRange.start(), minimumDuration, allDayTitles, timeZone,
                    availableTimes);
            if (occupiedRange.end().isAfter(availableStart)) {
                availableStart = occupiedRange.end();
            }
        }
        addAvailableTime(availableStart, windowEnd, minimumDuration, allDayTitles, timeZone, availableTimes);
    }

    private void addAvailableTime(
            Instant start, Instant end, Duration minimumDuration, List<String> allDayTitles, ZoneId timeZone,
    ...
    private void addExpandedOccurrences(
            RecurrenceEvent recurrenceEvent, Instant from, Instant to, Set<OccurrenceKey> responseKeys,
    ...
    private record TimeRange(Instant start, Instant end) {
        private static TimeRange overlapping(Instant start, Instant end, Instant rangeStart, Instant rangeEnd) {
            return new TimeRange(start.isAfter(rangeStart) ? start : rangeStart,
                    end.isBefore(rangeEnd) ? end : rangeEnd);
        }
