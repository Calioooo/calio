...
public class GoogleCalendarIntegrationCommandService {
  ...
  public void markIntegrationSyncError(
      GoogleCalendarIntegration integration, String reason, Instant occurredAt) {
    integration.markSyncError(reason, occurredAt);
    integrationRepository.saveAndFlush(integration);
  }

  public void markConnectedIntegrationSyncError(Long accountId, String reason, Instant occurredAt) {
  ...
  public void replaceAccessToken(
      ...
      Instant accessTokenExpiresAt) {
    int updated =
        integrationRepository.updateAccessToken(
            integrationId,
  ...
  public boolean acquireOperationLease(
      Long accountId, String ownerToken, long leaseDurationSeconds) {
    return integrationRepository.acquireGoogleOperationLease(
            accountId, ownerToken, leaseDurationSeconds)
        == 1;
  }

  public void extendOperationLease(
      Long jobId, Long accountId, String ownerToken, long leaseDurationSeconds) {
    if (integrationRepository.renewOwnedGoogleOperationLease(
            jobId, accountId, ownerToken, leaseDurationSeconds)
        != 1) {
