...
public interface GoogleCalendarIntegrationRepository
  ...
  @Lock(LockModeType.PESSIMISTIC_WRITE)
  @Query(
      "select integration from GoogleCalendarIntegration integration where integration.id = :integrationId")
  ...
  int updateNextSyncToken(
      @Param("integrationId") Long integrationId, @Param("nextSyncToken") String nextSyncToken);
