...
public class GoogleCalendarRecurrenceEventLoader {
  ...
  public Optional<RecurrenceEventUpsert> loadRecurrenceEvent(
      Long integrationId, String recurrenceEventExternalId, GoogleCalendarSyncRunContext context) {
  ...
  private Optional<RecurrenceEventUpsert> requestAndRemember(
      Long integrationId, String recurrenceEventExternalId, GoogleCalendarSyncRunContext context) {
    Optional<GoogleCalendarEventResponse> response =
        eventRequestService.getEvent(integrationId, recurrenceEventExternalId, context);
    if (response.isEmpty()) {
      context.rememberRecurrenceEvent(recurrenceEventExternalId, MissingRecurrenceEvent.INSTANCE);
      return Optional.empty();
    }
    RecurrenceEventUpsert recurrenceEvent = recurrenceMapper.mapRecurrenceEvent(response.get());
    context.rememberRecurrenceEvent(
        recurrenceEventExternalId, new FoundRecurrenceEvent(recurrenceEvent));
