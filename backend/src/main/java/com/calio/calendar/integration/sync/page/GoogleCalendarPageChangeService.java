...
public class GoogleCalendarPageChangeService {
  ...
  public void applyNormalizedPage(
    ...
    GoogleCalendarIntegration integration =
        integrationQueryService.getIntegrationById(integrationId);
    ...
    for (NormalizedItem item : items) {
      switch (item) {
        case EventUpsert event -> {
          ...
          eventChangeService.applyUpsert(integration, event, cache, account, defaultTag);
        }
        case EventCancellation cancellation ->
            eventChangeService.applyCancellation(cancellation.externalEventId(), cache);
        case RecurrenceEventUpsert recurrenceEvent -> {
          ...
          recurrenceChangeService.applyUpsert(
              integration, recurrenceEvent, cache, account, defaultTag);
        }
        case RecurrenceEventCancellation cancellation ->
            recurrenceChangeService.applyCancellation(cancellation.externalEventId(), cache);
  ...
  private void removeRecurrenceEventWithSameExternalId(
      String externalEventId, GoogleCalendarPageRecordCache cache) {
  ...
  private GoogleCalendarPageRecordCache loadPageRecordCache(
      Long integrationId, List<NormalizedItem> items) {
    Set<String> eventExternalIds =
        items.stream().map(NormalizedItem::externalEventId).collect(Collectors.toSet());
    ...
    Map<GoogleCalendarRecurrenceOverrideKey, GoogleCalendarRecurrenceOverrideMapping>
        googleOverrideMappings =
            loadGoogleOverrideMappings(integrationId, recurrenceEventMappings, items);
    ...
    return new GoogleCalendarPageRecordCache(
        eventMappings, recurrenceEventMappings, googleOverrideMappings, recurrenceEventOverrides);
  }

  private Map<String, GoogleCalendarEventMapping> loadEventMappings(
      Long integrationId, Set<String> externalEventIds) {
    if (externalEventIds.isEmpty()) {
      return new HashMap<>();
    }
    return eventMappingQueryService
        .listEventMappings(
            integrationId, GoogleCalendarEventMapping.PRIMARY_CALENDAR_KEY, externalEventIds)
        .stream()
        .collect(
            Collectors.toMap(
  ...
  private Map<String, GoogleCalendarRecurrenceEventMapping> loadRecurrenceEventMappings(
      Long integrationId, Set<String> recurrenceEventExternalIds) {
    if (recurrenceEventExternalIds.isEmpty()) {
      return new HashMap<>();
    }
    return recurrenceMappingQueryService
        .listRecurrenceEventMappings(
            integrationId,
  ...
  private Map<GoogleCalendarRecurrenceOverrideKey, GoogleCalendarRecurrenceOverrideMapping>
    ...
    rejectOverrideExternalIdsUsedByDifferentRecurrenceEvent(
        integrationId, recurrenceEventExternalIdsByOverrideExternalId);
    Set<Long> recurrenceEventMappingIds =
        recurrenceEventMappings.values().stream()
    ...
    return recurrenceMappingQueryService.listOverrideMappings(recurrenceEventMappingIds).stream()
        .collect(
            Collectors.toMap(
                mapping ->
                    new GoogleCalendarRecurrenceOverrideKey(
                        mapping.getRecurrenceEventMapping().getId(), mapping.getExternalEventId()),
                Function.identity(),
                (left, right) -> left,
                HashMap::new));
  }
        ...
        .forEach(
            override -> {
              ...
              if (existingRecurrenceEventId != null
                  && !existingRecurrenceEventId.equals(override.recurrenceEventExternalId())) {
  ...
  private void rejectOverrideExternalIdsUsedByDifferentRecurrenceEvent(
      Long integrationId, Map<String, String> recurrenceEventExternalIdsByOverrideExternalId) {
    if (recurrenceEventExternalIdsByOverrideExternalId.isEmpty()) {
      return;
    }
    recurrenceMappingQueryService
        .listOverrideMappings(
            integrationId,
            GoogleCalendarRecurrenceEventMapping.PRIMARY_CALENDAR_KEY,
        ...
        .forEach(
            mapping -> {
              String expectedRecurrenceEventId =
                  recurrenceEventExternalIdsByOverrideExternalId.get(mapping.getExternalEventId());
  ...
  private Map<RecurrenceEventOverrideKey, RecurrenceEventOverride> loadRecurrenceEventOverrides(
      Map<String, GoogleCalendarRecurrenceEventMapping> recurrenceEventMappings,
      List<NormalizedItem> items) {
    Map<RecurrenceEventOverrideKey, RecurrenceEventOverride> recurrenceEventOverrides =
        new HashMap<>();
    Map<String, List<RecurrenceEventOverrideUpsert>> overridesByRecurrenceEvent =
        items.stream()
            ...
            .map(RecurrenceEventOverrideUpsert.class::cast)
            .collect(
                Collectors.groupingBy(RecurrenceEventOverrideUpsert::recurrenceEventExternalId));
    overridesByRecurrenceEvent.forEach(
        (recurrenceEventExternalId, overrideItems) -> {
          ...
          Set<Instant> originStartTimes =
              overrideItems.stream()
                  .map(RecurrenceEventOverrideUpsert::originStartAt)
                  .collect(Collectors.toSet());
          recurrenceEventQueryService
              .listOverrides(recurrenceEventMapping.getRecurrenceEvent().getId(), originStartTimes)
              .forEach(
