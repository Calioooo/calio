...
public class GoogleCalendarRecurrenceJobHandler {
  ...
  private void createRecurrence(GoogleCalendarRecurrenceJob job, String workerToken) {
    ...
    RecurrenceMappingSnapshot existingMapping =
        mappingQueryService
      ...
      GoogleCalendarEventResponse created =
          eventsClient.post(
              ...
              GoogleCalendarEventWriteRequest.forRecurrenceCreate(
                  payload, job.getProviderIdentity()));
  ...
  private void deleteRecurrence(GoogleCalendarRecurrenceJob job, String workerToken) {
    ...
    try {
      eventsClient.delete(
          accessTokenService.getAccessToken(connectionId), mapping.externalId(), mapping.etag());
  ...
  private void patchRecurrence(
    ...
    GoogleCalendarEventResponse current =
        eventsClient.getEvent(token, mapping.externalId()).orElse(null);
  ...
  private void applyOverrideUpdate(
    ...
    GoogleCalendarEventResponse providerRecurrenceEvent =
        eventsClient.getEvent(token, recurrenceEventMapping.externalId()).orElse(null);
    if (providerRecurrenceEvent == null
        || !recurrenceEventMapping.etag().equals(providerRecurrenceEvent.etag())) {
      completeRecurrenceEventConflictForOverride(
    ...
    try {
      ...
      transactionTemplate.executeWithoutResult(
          status ->
              completeOverrideUpdate(
                  job,
                  workerToken,
                  recurrenceEventMapping.mappingId(),
                  occurrence.id(),
                  expected,
                  updated.etag()));
    } catch (GoogleCalendarEventVersionConflictException exception) {
  ...
  private void applyOverrideDelete(
      GoogleCalendarRecurrenceJob job, String workerToken, OverrideMappingScope scope) {
    RecurrenceMappingSnapshot recurrenceEventMapping = scope.recurrenceEventMapping();
    String token = accessTokenService.getAccessToken(recurrenceEventMapping.connectionId());
    GoogleCalendarEventResponse providerRecurrenceEvent =
        eventsClient.getEvent(token, recurrenceEventMapping.externalId()).orElse(null);
    ...
    try {
      ...
      transactionTemplate.executeWithoutResult(
          status -> completeOverrideDelete(job, workerToken, recurrenceEventMapping.mappingId()));
    } catch (GoogleCalendarEventVersionConflictException exception) {
      completeOverrideConflict(
          job, workerToken, recurrenceEventMapping.mappingId(), occurrence.id(), expected);
    }
  ...
  private OverrideMappingScope loadConnectedOverrideScope(GoogleCalendarRecurrenceJob job) {
    ...
    RecurrenceMappingSnapshot recurrenceEvent =
        mappingQueryService
            .getRecurrenceEventMappingIfExists(connectionId, job.getRecurrenceEventId())
            .map(mapping -> RecurrenceMappingSnapshot.from(mapping, connectionId))
            .orElse(null);
  ...
  private void createRecurrenceMappingIfAbsent(
      GoogleCalendarRecurrenceJob job, Long connectionId, GoogleCalendarEventResponse created) {
    if (mappingQueryService
        .getRecurrenceEventMappingIfExists(connectionId, job.getRecurrenceEventId())
        .isPresent()) {
      return;
    }
    connectionQueryService
        .getConnectionIfExists(connectionId)
        .ifPresent(
            connection ->
                mappingCommandService.createRecurrenceEventMapping(
                    new GoogleCalendarRecurrenceEventMapping(
                        connection, job.getRecurrenceEventId(), created.id(), created.etag())));
  }

  private void completeRecurrenceEventLocally(GoogleCalendarRecurrenceJob job, String workerToken) {
    transactionTemplate.executeWithoutResult(
        status -> {
          markInactiveRecurrenceEventMappingsLocalChanged(job);
          jobService.succeed(job.getId(), job.getAccountId(), workerToken);
  ...
  private void completeRecurrenceEventConflictForOverride(
      GoogleCalendarRecurrenceJob job, String workerToken, Long recurrenceEventMappingId) {
    transactionTemplate.executeWithoutResult(
        status -> {
  ...
  private void completeOverrideWithoutProviderWrite(
      GoogleCalendarRecurrenceJob job, String workerToken, boolean conflicted) {
    transactionTemplate.executeWithoutResult(
        status -> {
          markInactiveOverrideMappingsLocalChanged(job);
          if (conflicted) {
  ...
  private void completeOverrideUpdate(
    ...
    GoogleCalendarRecurrenceOverrideMapping override =
        mappingQueryService
            ...
            .orElse(null);
    if (override == null) {
      mappingCommandService.createOverrideMapping(
  ...
  private record RecurrenceMappingSnapshot(
      Long mappingId, Long connectionId, String externalId, String etag, boolean conflicted) {
    static RecurrenceMappingSnapshot from(
        GoogleCalendarRecurrenceEventMapping mapping, Long connectionId) {
