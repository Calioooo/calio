...
public class GoogleCalendarConnectionService {
  ...
  public GoogleCalendarIntegrationResponse connect(Long accountId, String authorizationCode) {
    ...
    GoogleTokenResponse tokenResponse =
        googleOAuthClient.exchangeAuthorizationCode(authorizationCode);
    ...
    GoogleCalendarIntegration integration =
        registerConnection(
            accountId, userInfo, encryptedTokens, accessTokenExpiresAt, tokenReceivedAt);
  ...
  public GoogleCalendarIntegrationResponse getConnectionStatus(Long accountId) {
    return integrationQueryService
        .getIntegrationIfExists(accountId)
  ...
  public void disconnect(Long accountId) {
    String encryptedRefreshToken =
        disconnectTransaction.execute(status -> disconnectLocally(accountId, Instant.now(clock)));
  ...
  private String disconnectLocally(Long accountId, Instant disconnectedAt) {
    return integrationCommandService
  ...
  private void validateConfiguration() {
    if (!googleOAuthProperties.isConfigured()) {
      logIntegrationFailure(
          "validateConfiguration", ErrorCode.GOOGLE_CALENDAR_CONFIGURATION_MISSING);
  ...
  private GoogleCalendarIntegration retryRegistrationAfterRace(
    ...
    try {
      ...
      logIntegrationFailure(
          "registerConnection", ErrorCode.GOOGLE_CALENDAR_INTEGRATION_SAVE_FAILED, exception);
  ...
  private GoogleCalendarIntegration register(GoogleCalendarConnectionCredentials credentials) {
  ...
  private GoogleCalendarIntegration createOrReplace(
      GoogleCalendarConnectionCredentials credentials) {
    return integrationCommandService
        .tryLockIntegration(credentials.accountId())
  ...
  private GoogleCalendarIntegration replaceConnection(
      GoogleCalendarIntegration integration, GoogleCalendarConnectionCredentials credentials) {
  ...
  private void revokeTokenSafely(Long accountId, String encryptedRefreshToken) {
    try {
      ...
      log.warn(
          "Google Calendar token revoke failed after local disconnect. accountId={} causeType={}",
          accountId,
          exception.getClass().getSimpleName());
    }
  }

  private record EncryptedGoogleTokens(String refreshToken, String accessToken) {}
