...
public class GoogleOperationJobService {
  ...
  private static final Logger log = LoggerFactory.getLogger(GoogleOperationJobService.class);
  ...
  public GoogleOperationJob claimNextJob(Long accountId, Long integrationId, String workerToken) {
    ...
    if (!head.canBeClaimedAt(now)) {
      ...
      log.info(
          "Google operation head is not runnable. accountId={} jobId={} state={} delaySeconds={}",
          accountId,
          head.getId(),
          head.getState().name(),
          delaySeconds);
    ...
    log.info(
        "Google operation job claimed. accountId={} jobId={} state={} previousState={}",
        accountId,
        head.getId(),
        head.getState().name(),
        previousState);
  ...
  public void retry(GoogleOperationJob job, String workerToken, String reason) {
    ...
    try {
      jobCommandService.retryOperationJob(job, workerToken, Instant.now(clock).plus(delay), reason);
    } catch (GoogleOperationOwnershipLostException exception) {
      log.warn(
          "Google operation retry transition rejected. accountId={} jobId={} state=PENDING delaySeconds={}",
          job.getAccountId(),
          job.getId(),
          delay.toSeconds());
      throw exception;
    ...
    log.info(
        "Google operation job scheduled for retry. accountId={} jobId={} state=PENDING delaySeconds={}",
        job.getAccountId(),
        job.getId(),
        delay.toSeconds());
  }
  ...
  public void terminate(Long jobId, Long accountId, String workerToken, String reason) {
    try {
      ...
      log.warn(
          "Google operation terminal transition rejected. accountId={} jobId={} state=SYNC_ERROR",
          accountId,
          jobId);
      throw exception;
    }
    log.info(
        "Google operation job terminated. accountId={} jobId={} state=SYNC_ERROR",
        accountId,
        jobId);
  }
  ...
  public void succeed(Long jobId, Long accountId, String workerToken) {
    try {
      ...
      log.warn(
          "Google operation success deletion rejected. accountId={} jobId={} state=PROCESSING",
          accountId,
          jobId);
      throw exception;
    }
  ...
  public List<Long> findRecoverableAccountIds() {
    return jobQueryService.listRecoverableAccountIds(Instant.now(clock), RECOVERY_BATCH_SIZE);
  }

  @Transactional
  public int deleteTerminalBatch(Instant cutoff) {
    List<Long> ids = jobQueryService.listExpiredTerminalJobIds(cutoff, TERMINAL_CLEANUP_BATCH_SIZE);
