...
public class GoogleOperationProducerTransaction {
  ...
  @Transactional
  public <T> T mutate(
      Long accountId, Supplier<T> authorizedCanonicalMutation, OutboundJobDraft jobDraft) {
    T result = authorizedCanonicalMutation.get();
    GoogleCalendarIntegration integration =
        integrationCommandService.tryLockIntegration(accountId).orElse(null);
    if (integration == null) {
      return result;
    }
    jobCommandService.enqueueOperationJob(
        GoogleOperationJob.outbound(
            ...
            jobDraft.kind(),
            jobDraft.resourceScope(),
            jobDraft.resourceKey(),
            jobDraft.providerIdentity(),
            jobDraft.desiredPayload(),
            Instant.now(clock)));
