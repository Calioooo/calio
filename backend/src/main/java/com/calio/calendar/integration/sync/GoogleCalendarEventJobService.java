...
public class GoogleCalendarEventJobService {
  ...
  public void execute(GoogleCalendarEventJob job, String workerToken) {
    switch (job.getKind()) {
      ...
      default ->
          throw new IllegalArgumentException("Unsupported Google Event job kind: " + job.getKind());
    }
  }
  ...
  private void createEvent(GoogleCalendarEventJob job, String workerToken) {
    ...
    transactionTemplate.executeWithoutResult(
        status -> completeCreate(job, workerToken, mappingResults, targetConnectionId, result));
  }
  ...
  private void updateEvent(GoogleCalendarEventJob job, String workerToken) {
    ...
    List<MappingExecutionResult> mappingResults = patchMappedEvents(eventSnapshot, mappings);
    transactionTemplate.executeWithoutResult(
        status -> completeUpdate(job, workerToken, mappingResults));
  }
  ...
  private void deleteEvent(GoogleCalendarEventJob job, String workerToken) {
    ...
    transactionTemplate.executeWithoutResult(
        status -> completeDelete(job, workerToken, mappingResults));
  }
  ...
  private List<EventMappingSnapshot> loadMappingSnapshots(GoogleCalendarEventJob job) {
    return Objects.requireNonNull(
        transactionTemplate.execute(
            status ->
                mappingQueryService
                    .listEventMappingsForEvent(job.getIntegrationId(), job.getEventId())
                    .stream()
                    .map(EventMappingSnapshot::from)
                    .toList()));
  }

  private Long findCreationTarget(GoogleCalendarEventJob job, List<EventMappingSnapshot> mappings) {
  ...
  private List<MappingExecutionResult> patchMappedEvents(
      GoogleEventJobPayload eventSnapshot, List<EventMappingSnapshot> mappings) {
    return mappings.stream().map(mapping -> patchMappedEvent(eventSnapshot, mapping)).toList();
  }

  private MappingExecutionResult patchMappedEvent(
      GoogleEventJobPayload eventSnapshot, EventMappingSnapshot mapping) {
    ...
    GoogleCalendarEventResponse providerEvent =
        eventsClient.getEvent(accessToken, mapping.externalEventId()).orElse(null);
  ...
  private List<MappingExecutionResult> deleteMappedEvents(List<EventMappingSnapshot> mappings) {
    return mappings.stream().map(this::deleteMappedEvent).toList();
  }
  ...
  private void completeUpdate(
      GoogleCalendarEventJob job, String workerToken, List<MappingExecutionResult> mappingResults) {
  ...
  private void deleteCompletedMappings(
    ...
    List<GoogleCalendarEventMapping> completedMappings =
  ...
  private boolean completeConflictOrSkip(
      GoogleCalendarEventJob job, String workerToken, MappingOutcome outcome) {
  ...
  private MappingOutcome applyMappingResult(
      GoogleCalendarEventMapping mapping, MappingExecutionResult result) {
  ...
  private void createEventMapping(
    ...
    GoogleCalendarConnection connection =
        connectionQueryService.listConnections(job.getIntegrationId()).stream()
            ...
            .orElse(null);
    if (connection != null) {
      mappingCommandService.createEventMapping(
          new GoogleCalendarEventMapping(
              connection, job.getEventId(), createdEvent.id(), createdEvent.etag()));
    }
  }
  ...
  private record MappingExecutionResult(
    ...
    private static MappingExecutionResult applied(Long mappingId) {
      return new MappingExecutionResult(
          mappingId, MappingOutcome.APPLIED, false, null, null, false);
    }
    ...
    private static MappingExecutionResult updated(
        Long mappingId, String expectedProviderEtag, String updatedProviderEtag) {
      return new MappingExecutionResult(
          mappingId,
