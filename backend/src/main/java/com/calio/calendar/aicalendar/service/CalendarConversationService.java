...
public class CalendarConversationService {
  ...
  private static final List<String> UNSUPPORTED_MESSAGES =
      List.of(
    ...
    return transactionTemplate.execute(
        status -> {
          CalendarConversation conversation =
              conversationCommandService.createConversation(accountId, clock.instant());
          return conversation.getConversationId();
        });
  }

  public SendCalendarConversationMessageResponse sendMessage(
      Long accountId, String conversationId, String message, String timeZone) {
    validateMessage(message);
    ZoneId zoneId = parseTimeZone(timeZone);
    CalendarConversation conversation =
        conversationQueryService.getConversation(accountId, conversationId);
    List<CalendarConversationHistoryMessage> history =
        recordUserMessageAndGetRecentHistory(conversation.getId(), message);
    ...
    CalendarAssistantRequest request =
        new CalendarAssistantRequest(accountId, conversationId, zoneId, history);
  ...
  private SendCalendarConversationMessageResponse recordUnsupportedResponse(
      Long conversationId, String externalConversationId) {
    CalendarAssistantAnswer answer =
        new CalendarAssistantAnswer(selectUnsupportedMessage(), List.of(), List.of(), List.of());
  ...
  private SendCalendarConversationMessageResponse recordAssistantResponse(
      Long conversationId, String externalConversationId, CalendarAssistantAnswer answer) {
  ...
  private String selectUnsupportedMessage() {
    return UNSUPPORTED_MESSAGES.get(
        ThreadLocalRandom.current().nextInt(UNSUPPORTED_MESSAGES.size()));
  }
  ...
  private void recordUserMessage(Long conversationId, String message) {
    conversationCommandService.recordMessage(
        conversationId, CalendarConversationMessageRole.USER, message, null, clock.instant());
  }
  ...
  private void recordAssistantMessage(
      Long conversationId, String message, List<CalendarAssistantBlockResponse<?>> responseBlocks) {
  ...
  public int deleteInactiveConversations(Instant cutoff) {
    return transactionTemplate.execute(
        status -> conversationCommandService.deleteInactiveConversations(cutoff));
  }
