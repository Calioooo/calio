...
public record CalendarMutationPreviewResponse(
  ...
  public record CalendarMutationEventResponse(
      String title, Instant startAt, Instant endAt, boolean allDay, TagResponse tag) {
    ...
    private static CalendarMutationEventResponse from(EventResponse event) {
      ...
      return new CalendarMutationEventResponse(
          event.title(), event.startAt(), event.endAt(), event.allDay(), event.tag());
    }
  }

  public record CalendarMutationRecurrencePreviewResponse(List<String> before, List<String> after) {
