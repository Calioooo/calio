...
public class CalendarAgentTools {
  ...
  @Tool(
      ...
      description =
          "Look up the authenticated user's current Calio calendar events for an inclusive ISO date range. Use this before a calendar update or deletion when the event ID must be resolved, and use its returned ID for a later mutation Preview.")
  public List<EventResponse> lookupCalendarEvents(
      CalendarLookupToolRequest request, ToolContext toolContext) {
    return runTool(
        LOOKUP_TOOL_NAME,
        toolContext,
        requestContext -> {
          CalendarToolTimeRange range =
              CalendarToolTimeRange.from(
                  request.startDate(), request.endDate(), requestContext.timeZone(), properties);
  ...
  @Tool(
      ...
      description =
          "Find free time in the authenticated user's current Calio calendar for an inclusive ISO date range, daily local time window, and minimum duration in minutes.")
  public FreeTimeSearchToolResult findCalendarFreeTime(
      FreeTimeSearchToolRequest request, ToolContext toolContext) {
    return runTool(
        FREE_TIME_TOOL_NAME,
        toolContext,
        requestContext -> {
  ...
  public CalendarMutationPreview previewCalendarMutation(
    ...
    return runTool(
        PREVIEW_MUTATION_TOOL_NAME,
        toolContext,
        requestContext -> {
          CalendarMutationPreview preview =
              mutationService.preview(requestContext.accountId(), request);
