...
public class SpringAiCalendarAssistantAgent implements AutoCloseable {
  ...
  private CalendarAssistantAnswer requestProviderAnswer(CalendarAssistantRequest request) {
    ...
    Map<String, Object> toolContext =
        calendarAgentTools.createToolContext(
            new CalendarAgentToolRequestContext(
                request.accountId(), request.conversationId(), request.timeZone()),
            resultCollector);
    String responseState = responseState(request.history());
    String answer =
        requestProviderWithRetry(
            chatModel,
  ...
  private boolean shouldRetryProviderRequest(
      int attempt, RuntimeException exception, long runDeadlineNanos) {
        ...
        providerExecutor.submit(
            () ->
                ChatClient.create(chatModel)
                    .prompt()
                    .system(system -> system.text(systemPrompt).params(systemPromptParameters))
                    .messages(history)
  ...
  private List<Message> conversationMessages(List<CalendarConversationHistoryMessage> history) {
    return history.stream().map(this::conversationMessage).toList();
  }
