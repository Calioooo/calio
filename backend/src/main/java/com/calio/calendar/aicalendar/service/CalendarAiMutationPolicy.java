...
public class CalendarAiMutationPolicy {
  ...
  private static final DateTimeFormatter UNTIL_FORMATTER =
      DateTimeFormatter.ofPattern("yyyyMMdd'T'HHmmss'Z'");
  private static final Pattern SIMPLE_RECURRENCE_RULE =
      Pattern.compile("RRULE:FREQ=(DAILY|WEEKLY|MONTHLY|YEARLY)(?:;UNTIL=(\\d{8}(?:T\\d{6}Z)?))?");
  ...
  public void validateCreateRecurrence(List<String> recurrenceRules, RecurrenceSchedule schedule) {
  ...
  private void requireAllowedEndDate(String until, RecurrenceSchedule schedule) {
    ...
    if (endDate.isAfter(maximumEndDate())
        || endDate.isBefore(schedule.firstOccurrenceLocalStart().toLocalDate())) {
      throw new CalioException(ErrorCode.INVALID_RECURRENCE_SCHEDULE);
    }
  ...
  private void requireUntilMatchesFirstOccurrenceTime(
      String until, LocalDate endDate, RecurrenceSchedule schedule) {
