...
public record SendCalendarConversationMessageResponse(
  ...
  public static SendCalendarConversationMessageResponse from(
      String conversationId, CalendarAssistantAnswer answer) {
    ...
    if (!answer.freeTimes().isEmpty()) {
      blocks.add(
          CalendarAssistantBlockResponse.freeTimes(
              answer.freeTimes().stream().map(FreeTimeResponse::from).toList()));
    }
    if (!answer.mutationPreviews().isEmpty()) {
      blocks.add(
          CalendarAssistantBlockResponse.mutationPreviews(
              answer.mutationPreviews().stream()
