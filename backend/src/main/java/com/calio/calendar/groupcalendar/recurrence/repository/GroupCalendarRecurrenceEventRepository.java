...
public interface GroupCalendarRecurrenceEventRepository
    extends JpaRepository<GroupCalendarRecurrenceEvent, Long> {
  ...
  Optional<GroupCalendarRecurrenceEvent> findByIdAndGroupSpaceIdForUpdate(
      @Param("id") Long id, @Param("groupSpaceId") Long groupSpaceId);
  ...
  List<GroupCalendarRecurrenceEvent> findByGroupSpaceIdAndFirstOccurrenceStartAtBefore(
      @Param("groupSpaceId") Long groupSpaceId, @Param("to") Instant to);
