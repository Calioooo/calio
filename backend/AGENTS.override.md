...
- 새 코드와 수정 코드는 기존 backend의 Java 포맷, 줄바꿈, 들여쓰기, import 정렬 방식을 따른다. 한 줄에 여러 선언이나 제어 흐름을 압축하지 않는다.
- 사용 타입은 파일 상단에 명시적으로 import하고, 코드 본문에서 완전 수식 클래스명을 사용하지 않는다. 사용하지 않는 import와 wildcard import는 남기지 않는다.
