...
@MainActor final class GroupSpaceDetailViewModel: ObservableObject {
  ...
  func update(name: String) async -> Bool {
    do {
      groupSpace = try await service.update(
        groupSpaceId: groupSpace.groupSpaceId, name: name, emoji: groupSpace.emoji)
      clearFailure()
      return true
    } catch is CancellationError {
      return false
  ...
  func remove(member: GroupMember) async -> Bool {
    ...
    do {
      try await service.removeMember(
        groupSpaceId: groupSpace.groupSpaceId, memberId: member.memberId)
