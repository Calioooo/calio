...
final class CalendarHomeViewModel: ObservableObject {
  ...
  func loadInitialIfNeeded() {
    guard state.daysByKey.isEmpty && monthEventCache.isEmpty && monthHolidayCache.isEmpty else {
      return
    }
  ...
  func moveMonth(by value: Int) {
    ...
    guard
      let movedMonthDate = calendar.date(
        byAdding: .month,
        value: value,
        to: currentDate
      )
  ...
  func updateRecurrenceOccurrence(_ event: Event, input: EventUpdateInput) async -> Bool {
    guard let recurrenceId = event.recurrenceId,
      let originStartAt = event.originStartAt
    else {
      return false
    }
  ...
  private func loadedDateRangeAfterAppending(
    ...
    guard loadedMonthCount(from: appendedStartDate, to: appendedEndDate) > maxLoadedMonthCount
    else {
      return (
