...
@MainActor final class GroupInvitationViewModel: ObservableObject {
  ...
  func accept(type: GroupInvitationCredentialKind, credential: String, nickname: String) async
    -> Bool
  {
    isSubmitting = true
    defer { isSubmitting = false }
    do {
      acceptanceResult = try await service.accept(
        type: type, credential: credential, nickname: nickname)
      errorMessage = nil
      return true
    } catch {
