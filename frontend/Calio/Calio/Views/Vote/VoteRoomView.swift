...
struct VoteRoomView: View {
  ...
  var body: some View {
    ...
    .onChange(of: viewModel.participantFlow) { _, flow in
      if flow != .result { resultDayForPopover = nil }
    }
  ...
  private func showResultPopover(for day: VoteDay) {
    guard resultDayForPopover == nil,
      let result = viewModel.result?.dateResults.first(where: { $0.day == day }),
