...
private struct MonthEventSpanView: View {
  ...
  var body: some View {
    ...
    .accessibilityLabel(
      "\(span.chip.title), \(span.chip.accessibilityKind)\(span.chip.isImportant ? ", 중요 일정" : "")"
    )
...
private struct CalendarDayDetailEventRow: View {
  ...
  var body: some View {
    ...
    .accessibilityLabel(
      "\(event.title), \(event.importantEvent ? "중요 일정, " : "")\(eventTimeText)\(hasDescription ? ", \(event.description)" : "")"
    )
