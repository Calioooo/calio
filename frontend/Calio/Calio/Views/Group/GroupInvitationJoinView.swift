...
struct GroupInvitationJoinView: View {
  ...
  private func previewConfirmation(_ preview: GroupInvitationPreview) -> some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      Button {
        Task {
          _ = await viewModel.accept(
            type: credentialType, credential: credential.trimmed, nickname: nickname.trimmed)
        }
      } label: {
  ...
  private func memberPreviewSummary(_ members: [GroupInvitationMemberPreview], totalCount: Int)
    -> String
  {
...
private struct GroupInvitationAcceptanceResultSheet: View {
  ...
  var body: some View {
    VStack(spacing: 16) {
      Image(
        systemName: result.joinResult == .alreadyMember
          ? "person.crop.circle.badge.checkmark" : "checkmark.circle.fill"
      )
      .font(.system(size: 40))
      .foregroundStyle(.calioBrand)
