    ...
    NavigationStack {
      ...
      .toolbar {
        ToolbarItem(placement: .topBarLeading) {
          Button {
            isPresentingInvitationJoin = true
          } label: {
        ...
        NavigationStack {
          ...
          .toolbar {
            ToolbarItem(placement: .cancellationAction) {
              Button("취소") { isPresentingCreation = false }
            }
            ToolbarItem(placement: .confirmationAction) {
              Button("만들기") {
                Task {
                  if await viewModel.create(name: groupName.trimmed, nickname: nickname.trimmed) {
    ...
    Binding(
      get: {
        viewModel.errorMessage != nil
          && !(viewModel.didFailLoading && viewModel.failedOperation == .load)
      },
