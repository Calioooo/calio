...
struct GroupSpaceDetailView: View {
    ...
    List {
      Section {
        HStack(spacing: 14) {
          ...
          VStack(alignment: .leading, spacing: 4) {
            ...
            Text("멤버 \(viewModel.groupSpace.memberCount)명").font(.subheadline).foregroundStyle(
              .calioTextSecondary)
          }
        }.padding(.vertical, 6)
      }
      Section("멤버") {
        if viewModel.isLoading && viewModel.members.isEmpty {
          ...
          ForEach(viewModel.members, id: \.memberId) { member in memberRow(member) }
        }
      }
      Section("그룹 설정") {
        groupSettings
        ...
        Form { TextField("그룹 이름", text: $editedName) }.navigationTitle("그룹 이름 수정")
          .toolbar {
            ...
            ToolbarItem(placement: .confirmationAction) {
              Button("완료") {
                Task {
                  if await viewModel.update(name: editedName.trimmed) {
  ...
  @ViewBuilder private func memberRow(_ member: GroupMember) -> some View {
    HStack {
      VStack(alignment: .leading, spacing: 3) {
        ...
        Text(member.role == .owner ? "소유자" : "멤버").font(.footnote).foregroundStyle(
          .calioTextSecondary)
      }
      Spacer()
  ...
  @ViewBuilder private var confirmationButtons: some View {
      ...
      Button("그룹 삭제", role: .destructive) {
        Task {
          if await viewModel.delete() {
            onFinished(.removed(viewModel.groupSpace.groupSpaceId))
            dismiss()
          }
        }
      }
    ...
    case .remove(let member):
      Button("멤버 내보내기", role: .destructive) {
        Task {
  ...
  private enum Confirmation {
    case delete, leave
    case remove(GroupMember)
    case transfer(GroupMember)
