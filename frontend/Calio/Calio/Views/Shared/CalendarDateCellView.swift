...
struct CalendarDateCellView: View {
  ...
  init(
    weekday: CalendarWeekday, dayText: String, isToday: Bool, isSelected: Bool = false,
    onTap: @escaping () -> Void, events: [Event]
  ) {
  ...
  private var visibleEvents: [Event] {
    let eventLimit =
      events.count > maxVisibleEventCount
...
#Preview {
  CalendarDateCellView(
    ...
    events: [
      Event(
        title: "제목", description: "설명", startAt: Date(), endAt: Date().addingTimeInterval(3000),
        tag: .sample(colorCode: "#FDDDDD")),
      Event(
        title: "제목", description: "설명", startAt: Date(), endAt: Date().addingTimeInterval(3000),
        tag: .sample(colorCode: "#RDDDDD")),
      Event(
        title: "제목", description: "설명", startAt: Date(), endAt: Date().addingTimeInterval(3000),
        tag: .sample(colorCode: "#BDDDDD")),
      Event(
        title: "제목", description: "설명", startAt: Date(), endAt: Date().addingTimeInterval(3000),
        tag: .sample(colorCode: "#ADDDDD")),
    ]
  )
}
