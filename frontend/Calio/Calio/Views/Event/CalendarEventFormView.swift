...
struct CalendarEventFormView: View {
  ...
  private var recurrenceSection: some View {
    if let recurrenceInput {
      Section("반복") {
        ...
        if mode.showsRecurrenceFrequency(
          isRecurrenceEnabled: recurrenceInput.wrappedValue.isEnabled)
        {
    ...
    Button {
      ...
      HStack {
        ...
        Text(recurrenceInput.wrappedValue.isEnabled ? "켜짐" : "꺼짐")
          ...
          .foregroundStyle(
            recurrenceInput.wrappedValue.isEnabled ? Color.calioBrand : Color.calioTextSecondary)
      }
  ...
  private func recurrenceFrequencyButton(
    ...
    Button {
      ...
      Text(frequency.koreanLabel)
        .font(.subheadline.weight(.semibold))
        .foregroundStyle(
          recurrenceInput.wrappedValue.frequency == frequency ? Color.white : Color.calioTextPrimary
        )
        ...
        .background(
          RoundedRectangle(cornerRadius: 8)
            .fill(
              recurrenceInput.wrappedValue.frequency == frequency
                ? Color.calioBrand : Color.calioSelection)
        )
        .overlay {
          RoundedRectangle(cornerRadius: 8)
  ...
  private func setAllDay(_ isAllDay: Bool) {
    ...
    if isAllDay {
      ...
      eventInput.endAt =
        calendar.date(byAdding: .day, value: 1, to: inclusiveEndDate)
        ?? inclusiveEndDate
      ...
      let startDate = calendar.startOfDay(for: eventInput.startAt)
      eventInput.startAt =
        calendar.date(bySettingHour: 9, minute: 0, second: 0, of: startDate)
        ?? startDate
      eventInput.endAt =
        calendar.date(byAdding: .hour, value: 1, to: eventInput.startAt)
        ?? eventInput.startAt
    }
