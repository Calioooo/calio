...
struct CalendarWeekTimelineView: View {
  ...
  init(
    ...
    onUpdateRecurrenceSeries: @escaping (Int64, RecurrenceEventSeriesEditInput) async -> Bool = {
      _, _ in true
    },
  ...
  private func timelinePopoverPlacement(
    ...
    let canCenterHorizontally =
      anchor.x - size.width / 2 >= edgePadding
    ...
    VStack(alignment: .leading, spacing: 12) {
      ...
      VStack(spacing: 0) {
        ForEach(events) { event in
          ...
          .buttonStyle(.plain)
          .accessibilityLabel(
            "\(event.title), \(event.importantEvent ? "중요 일정, " : "")\(CalendarEventDisplayText.compactDateTimeRange(startAt: event.startAt, endAt: event.endAt))"
          )
...
#Preview {
  ...
  let makeEvent: (Int64, String, Date, Int, Int, Int, String) -> Event = {
    id, title, date, hour, minute, durationMinutes, colorCode in
