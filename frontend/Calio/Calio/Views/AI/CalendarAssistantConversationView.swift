...
struct CalendarAssistantConversationView: View {
  ...
  init(
    ...
    _viewModel = StateObject(
      wrappedValue: CalendarAssistantConversationViewModel(
    ...
    VStack(spacing: 0) {
      ...
      if let failure = viewModel.messageFailure {
        HStack(spacing: 10) {
          ...
          Spacer()
          Button("다시 보내기") { Task { await viewModel.retryMessageSend() } }.font(
            .footnote.weight(.semibold))
        }.padding(12).background(Color.calioSelection, in: RoundedRectangle(cornerRadius: 14))
          .padding(.horizontal, 16).padding(.bottom, 8).accessibilityIdentifier(
            "calendar_assistant_message_failed")
      }
      composer
    }
  }
  ...
  private var emptyConversation: some View {
    VStack(alignment: .leading, spacing: 10) {
      Image(systemName: "sparkles").font(.title2.weight(.semibold)).foregroundStyle(.calioPrimary)
        .frame(width: 44, height: 44).background(Color.calioSelection, in: Circle())
      Text("무엇을 도와드릴까요?").font(.headline).foregroundStyle(.calioTextPrimary)
      Text("일정을 찾거나, 빈 시간을 확인하고 일정 변경을 제안받아 보세요.").font(.subheadline).foregroundStyle(
        .calioTextSecondary)
    }.padding(18).background(Color.calioSurface, in: RoundedRectangle(cornerRadius: 18)).overlay(
      RoundedRectangle(cornerRadius: 18).stroke(Color.calioDivider, lineWidth: 1)
    ).padding(.top, 12)
  }

  private var composer: some View {
    HStack(alignment: .bottom, spacing: 10) {
      TextField("캘린더에 요청하기", text: $draft, axis: .vertical).lineLimit(1...4).padding(
        .horizontal, 12
      ).padding(.vertical, 10).background(
        Color.calioBackground, in: RoundedRectangle(cornerRadius: 14)
      ).overlay(RoundedRectangle(cornerRadius: 14).stroke(Color.calioDivider, lineWidth: 1))
        .accessibilityIdentifier("calendar_assistant_composer")
      Button {
        let value = draft
        draft = ""
        Task { await viewModel.send(value) }
      } label: {
        Image(systemName: viewModel.isSending ? "ellipsis" : "arrow.up").font(
          .headline.weight(.bold)
        ).frame(width: 42, height: 42).foregroundStyle(.white).background(
          Color.calioBrand, in: Circle())
      }.disabled(
        draft.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || viewModel.isSending
      ).accessibilityLabel("메시지 보내기")
    }.padding(.horizontal, 16).padding(.vertical, 12).background(Color.calioSurface).overlay(
      alignment: .top
    ) { Rectangle().fill(Color.calioDivider).frame(height: 1) }.shadow(
      color: .black.opacity(0.04), radius: 8, y: -2)
  }
    ...
    VStack(alignment: message.role == .user ? .trailing : .leading, spacing: 10) {
      HStack(spacing: 8) {
        if message.role == .assistant {
          Image(systemName: "sparkles").font(.caption.weight(.bold)).foregroundStyle(.calioPrimary)
        }
        Text(message.role == .user ? "나" : "AI 캘린더").font(.caption.weight(.semibold))
          .foregroundStyle(message.role == .user ? Color.calioTextSecondary : Color.calioPrimary)
      }
      messageText(message).font(.subheadline).foregroundStyle(
        message.role == .user ? .white : .calioTextPrimary
      ).padding(.horizontal, 14).padding(.vertical, 12).background(
        message.role == .user ? Color.calioBrand : Color.calioSurface,
        ...
        message.role == .user ? "내 메시지: \(message.text)" : "AI 답변: \(message.text)")
  ...
  private func mutationPreviewView(_ preview: CalendarAssistantMutationPreview) -> some View {
    VStack(alignment: .leading, spacing: 12) {
      HStack(alignment: .top) {
        ...
        Spacer()
        Text(mutationTypeLabel(preview.type)).font(.caption.weight(.bold)).foregroundStyle(
          .calioPrimary
        ).padding(.horizontal, 8).padding(.vertical, 4).background(
          Color.calioSurface, in: Capsule())
      }
      Text(mutationScopeLabel(preview.scope)).font(.caption).foregroundStyle(.calioTextSecondary)
      if let before = preview.before {
        mutationEventView(before, label: preview.after == nil ? "삭제될 일정" : "변경 전")
      }
      if preview.before != nil, preview.after != nil { Divider() }
      if let after = preview.after {
        mutationEventView(after, label: preview.before == nil ? "새 일정" : "변경 후")
      }
      if let recurrenceBefore = preview.recurrenceBefore,
        let recurrenceAfter = preview.recurrenceAfter
      {
        Divider()
    ...
    -> some View
  {
    VStack(alignment: .leading, spacing: 6) {
      ...
      Label(
        ...
        systemImage: event.allDay ? "sun.max" : "clock"
      )
  ...
  private func mutationScopeLabel(_ scope: String) -> String {
    switch scope {
    case "EVENT": return "이 일정"
    case "THIS_OCCURRENCE": return "이번 일정"
  ...
  private var messageAnimation: Animation {
    reduceMotion ? .easeOut(duration: 0.12) : .spring(response: 0.35, dampingFraction: 0.86)
  }
...
extension View {
  fileprivate func resultCard(title: String, icon: String, label: String) -> some View {
    VStack(alignment: .leading, spacing: 10) {
      HStack(spacing: 7) {
        Image(systemName: icon).foregroundStyle(.calioPrimary)
        Text(title).font(.caption.weight(.bold)).foregroundStyle(.calioTextPrimary)
      }
      self
    }.padding(14).background(Color.calioSelection, in: RoundedRectangle(cornerRadius: 16)).overlay(
      RoundedRectangle(cornerRadius: 16).stroke(Color.calioDivider.opacity(0.75), lineWidth: 1)
