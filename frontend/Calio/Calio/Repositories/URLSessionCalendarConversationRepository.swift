...
struct URLSessionCalendarConversationRepository: CalendarConversationRepository {
    ...
    func createConversation() async throws -> CreateCalendarConversationResponseDTO {
        try await apiClient.send(CreateCalendarConversationResponseDTO.self, method: .post, pathComponents: ["api", "ai", "calendar", "conversations"], authorization: .bearer)
    }
    func sendMessage(conversationId: String, request: SendCalendarConversationMessageRequestDTO) async throws -> SendCalendarConversationMessageResponseDTO {
        try await apiClient.send(SendCalendarConversationMessageResponseDTO.self, method: .post, pathComponents: ["api", "ai", "calendar", "conversations", conversationId, "messages"], authorization: .bearer, body: request)
    }
}
