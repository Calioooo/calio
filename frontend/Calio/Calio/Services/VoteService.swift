...
struct VoteService {
    ...
    func createRoom(name: String, candidateEndDay: VoteDay) async throws -> VoteRoom {
        let request = CreateVoteRoomRequestDTO(name: name, candidateEndDate: candidateEndDay.apiDateString)
    ...
    private func mapVoteParticipantSelection(
        ...
        VoteParticipantSelection(
            participant: VoteParticipant(nickname: dto.nickname, status: mapVoteParticipantStatus(dto.status)),
