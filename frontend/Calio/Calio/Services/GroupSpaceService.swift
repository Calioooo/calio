...
struct GroupSpaceService {
  ...
  func fetchGroupSpace(groupSpaceId: Int64) async throws -> GroupSpace {
    let response = try await performGroupSpaceRequest {
      try await repository.fetchGroupSpace(groupSpaceId: groupSpaceId)
    }
  ...
  func update(groupSpaceId: Int64, name: String, emoji: String?) async throws -> GroupSpace {
    let response = try await performGroupSpaceRequest {
      try await repository.updateGroupSpace(
        groupSpaceId: groupSpaceId, request: .init(name: name, emoji: emoji))
    }
    return mapToGroupSpace(response)
  }
  ...
  func members(groupSpaceId: Int64) async throws -> [GroupMember] {
    let response = try await performGroupSpaceRequest {
      try await repository.fetchMembers(groupSpaceId: groupSpaceId)
    }
  ...
  func transferOwnership(groupSpaceId: Int64, targetMemberId: Int64) async throws
    -> GroupOwnershipTransfer
  {
    let response = try await performGroupSpaceRequest {
      try await repository.transferOwnership(
        groupSpaceId: groupSpaceId, request: .init(targetMemberId: targetMemberId))
    }
  ...
  private func mapToGroupMember(_ dto: GroupMemberResponseDTO) -> GroupMember {
    GroupMember(
      memberId: dto.memberId, nickname: dto.nickname, role: mapToGroupMemberRole(dto.role))
  }
