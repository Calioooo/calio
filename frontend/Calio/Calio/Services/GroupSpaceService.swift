...
struct GroupSpaceService {
    ...
    func fetchGroupSpaces() async throws -> [GroupSpace] {
        let response = try await performGroupSpaceRequest { try await repository.fetchGroupSpaces() }
        return response.groupSpaces.map(mapToGroupSpace(_:))
    }
    func fetchGroupSpace(groupSpaceId: Int64) async throws -> GroupSpace {
        let response = try await performGroupSpaceRequest { try await repository.fetchGroupSpace(groupSpaceId: groupSpaceId) }
        return mapToGroupSpace(response)
    }
    func create(name: String, emoji: String?, nickname: String) async throws -> GroupSpace {
        let response = try await performGroupSpaceRequest { try await repository.createGroupSpace(.init(name: name, emoji: emoji, nickname: nickname)) }
        return mapToGroupSpace(response)
    }
    func update(groupSpaceId: Int64, name: String, emoji: String?) async throws -> GroupSpace {
        let response = try await performGroupSpaceRequest { try await repository.updateGroupSpace(groupSpaceId: groupSpaceId, request: .init(name: name, emoji: emoji)) }
        return mapToGroupSpace(response)
    }
    func delete(groupSpaceId: Int64) async throws {
        try await performGroupSpaceRequest { try await repository.deleteGroupSpace(groupSpaceId: groupSpaceId) }
    }
    func members(groupSpaceId: Int64) async throws -> [GroupMember] {
        let response = try await performGroupSpaceRequest { try await repository.fetchMembers(groupSpaceId: groupSpaceId) }
        return response.members.map(mapToGroupMember(_:))
    }
    func leave(groupSpaceId: Int64) async throws {
        try await performGroupSpaceRequest { try await repository.leaveGroupSpace(groupSpaceId: groupSpaceId) }
    }
    func removeMember(groupSpaceId: Int64, memberId: Int64) async throws {
        try await performGroupSpaceRequest { try await repository.removeMember(groupSpaceId: groupSpaceId, memberId: memberId) }
    }
    func transferOwnership(groupSpaceId: Int64, targetMemberId: Int64) async throws -> GroupOwnershipTransfer {
        let response = try await performGroupSpaceRequest { try await repository.transferOwnership(groupSpaceId: groupSpaceId, request: .init(targetMemberId: targetMemberId)) }
...
func performGroupSpaceRequest<T>(_ operation: () async throws -> T) async throws -> T {
    do {
