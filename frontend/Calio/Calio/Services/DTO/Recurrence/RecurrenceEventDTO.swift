...
struct CreateRecurrenceEventRequestDTO: Encodable, Equatable {
  ...
  init(
    title: String, description: String?, allDay: Bool, firstOccurrenceStartAt: Date,
    firstOccurrenceEndAt: Date, timeZone: String?, recurrence: [String], tagId: Int64?
  ) {
    self.title = title
  ...
  init(
    recurrenceTitle: String, recurrenceDescription: String?, recurrenceStartDate: String,
    recurrenceEndDate: String, recurrenceStartTime: String, recurrenceEndTime: String,
    recurrenceFrequency: RecurrenceFrequency, tagId: Int64? = nil
  ) {
    self.init(
      ...
      timeZone: "UTC",
      recurrence: [
        "RRULE:FREQ=\(recurrenceFrequency.rawValue);UNTIL=\(recurrenceEndDate.replacingOccurrences(of: "-", with: ""))T000000Z"
      ],
      tagId: tagId
    )
  }
  ...
  var recurrenceEndTime: String {
    allDay ? "23:59:59" : Self.legacyTimeString(firstOccurrenceEndAt)
  }
...
struct UpdateRecurrenceEventRequestDTO: Encodable, Equatable {
  ...
  init(
    title: String, description: String?, startDate: String, endDate: String, startTime: String,
    endTime: String, recurrenceFrequency: RecurrenceFrequency, tagId: Int64? = nil
  ) {
    self.init(
      title: title,
      ...
      timeZone: "UTC",
      recurrence: [
        "RRULE:FREQ=\(recurrenceFrequency.rawValue);UNTIL=\(endDate.replacingOccurrences(of: "-", with: ""))T000000Z"
      ],
      tagId: tagId
    )
  }
  ...
  var endDate: String {
    CreateRecurrenceEventRequestDTO.legacyUntilDate(in: recurrence) ?? startDate
  }
  var startTime: String { CreateRecurrenceEventRequestDTO.legacyTimeString(firstOccurrenceStartAt) }
  var endTime: String {
    allDay ? "23:59:59" : CreateRecurrenceEventRequestDTO.legacyTimeString(firstOccurrenceEndAt)
  }
...
struct UpdateRecurrenceOccurrenceRequestDTO: Encodable, Equatable {
  ...
  init(
    originStartAt: Date, title: String, description: String?, startAt: Date, endAt: Date,
    allDay: Bool, timeZone: String?
  ) {
  ...
  init(originStartAt: Date, startAt: Date, endAt: Date) {
    self.init(
      originStartAt: originStartAt, title: "", description: nil, startAt: startAt, endAt: endAt,
...
struct RecurrenceEventResponseDTO: Decodable, Equatable {
  ...
  init(
    recurrenceId: Int64, recurrenceTitle: String, recurrenceDescription: String?,
    recurrenceStartDate: String, recurrenceEndDate: String, recurrenceStartTime: String,
    recurrenceEndTime: String, recurrenceFrequency: RecurrenceFrequency,
    tag: TagResponseDTO = TagResponseDTO(
      id: 0, title: "기타", colorCode: "#64748B", tagType: .defaultTag)
  ) {
    self.init(
      ...
      firstOccurrenceStartAt: CreateRecurrenceEventRequestDTO.legacyDate(
        recurrenceStartDate, recurrenceStartTime),
      firstOccurrenceEndAt: CreateRecurrenceEventRequestDTO.legacyDate(
        recurrenceStartDate, recurrenceEndTime),
      timeZone: "UTC",
      recurrence: [
...
extension CreateRecurrenceEventRequestDTO {
  ...
  fileprivate static func legacyDateString(_ date: Date) -> String {
    let formatter = DateFormatter()
    formatter.locale = Locale(identifier: "en_US_POSIX")
    formatter.timeZone = TimeZone(secondsFromGMT: 0)
  ...
  fileprivate static func legacyTimeString(_ date: Date) -> String {
    let formatter = DateFormatter()
    formatter.locale = Locale(identifier: "en_US_POSIX")
    formatter.timeZone = TimeZone(secondsFromGMT: 0)
  ...
  fileprivate static func legacyUntilDate(in recurrence: [String]) -> String? {
    guard
  ...
  fileprivate static func legacyFrequency(in recurrence: [String]) -> RecurrenceFrequency? {
    guard
      let frequency = recurrence
        .first?
        .split(separator: ";")
        .first(where: { $0.hasPrefix("RRULE:FREQ=") })?
        .dropFirst("RRULE:FREQ=".count)
    else {
      return nil
    }
