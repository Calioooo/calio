...
enum GroupMemberRoleDTO: String, Decodable { case owner = "OWNER", member = "MEMBER" }
...
struct GroupSpaceListResponseDTO: Decodable { let groupSpaces: [GroupSpaceResponseDTO] }
struct GroupMemberResponseDTO: Decodable { let memberId: Int64; let nickname: String; let role: GroupMemberRoleDTO }
struct GroupMemberListResponseDTO: Decodable { let members: [GroupMemberResponseDTO] }
struct CreateGroupSpaceRequestDTO: Encodable { let name: String; let emoji: String?; let nickname: String }
struct UpdateGroupSpaceRequestDTO: Encodable { let name: String; let emoji: String? }
struct TransferGroupOwnerRequestDTO: Encodable { let targetMemberId: Int64 }
struct TransferGroupOwnerResponseDTO: Decodable { let previousOwner: GroupMemberResponseDTO; let owner: GroupMemberResponseDTO }
struct GroupInvitationResponseDTO: Decodable { let invitationId: Int64; let inviteUrl: String; let inviteCode: String; let expiresAt: Date }
struct GroupInvitationSummaryResponseDTO: Decodable { let invitationId: Int64; let expiresAt: Date }
struct GroupInvitationListResponseDTO: Decodable { let invitations: [GroupInvitationSummaryResponseDTO] }
struct PreviewGroupInvitationRequestDTO: Encodable { let credentialType: GroupInvitationPreviewCredentialTypeDTO; let credential: String }
struct GroupInvitationMemberPreviewDTO: Decodable { let nickname: String }
