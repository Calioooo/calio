...
struct LocalEventTextParser {
  ...
  func parse(
    ...
    let recurrenceFrequency = parseRecurrenceFrequency(
      in: trimmedText, matchedRanges: &matchedRanges)
  ...
  private func parseDateRange(
    ...
    if let match = firstMatch(
      in: text,
      pattern:
        #"(?<!\d)(\d{1,2})\s*/\s*(\d{1,2})\s*(?:-|~)\s*(\d{1,2})\s*/\s*(\d{1,2})(?!\d)(?:\s*동안)?"#
  ...
  private func parseTime(
    ...
    if let match = firstMatch(
      in: text,
      pattern:
        #"(오전|오후|아침|저녁|밤)?\s*(\d{1,2})(?::(\d{2})|시(?:\s*(\d{1,2})분)?)\s*(?:부터|-|~)\s*(오전|오후|아침|저녁|밤)?\s*(\d{1,2})(?::(\d{2})|시(?:\s*(\d{1,2})분)?)(?:\s*까지)?"#
  ...
  private func dateRange(
    ...
    guard let referenceYear = referenceComponents.year,
      ...
      var startDate = exactDate(year: referenceYear, month: referenceMonth, day: startDay)
    else {
      return nil
    }
  ...
  private func dateInWeek(
    ...
    guard let weekStart = mondayStartOfWeek(containing: date),
      let targetWeekStart = calendar.date(byAdding: .weekOfYear, value: weekOffset, to: weekStart)
    else {
      return nil
    }
