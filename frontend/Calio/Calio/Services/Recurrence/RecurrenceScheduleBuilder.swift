...
enum RecurrenceScheduleBuilder {
  static func make(
    ...
    if allDay {
      let firstEndDate =
        Calendar(identifier: .gregorian).date(byAdding: .day, value: 1, to: startDate) ?? startDate
    ...
    let until = try endDate.map { date in
      guard
        let value = instant(
          date: date, time: startTime, timeZone: timeZone, formTimeZone: formTimeZone)
      else {
        throw EventServiceError.validationFailed
      }
      return value
  ...
  private static func instant(
    ...
    guard let candidate,
      targetCalendar.dateComponents([.year, .month, .day], from: candidate) == day
    else {
      return nil
    }
    return candidate
