...
struct GroupInvitationService {
    ...
    func issue(groupSpaceId: Int64) async throws -> IssuedGroupInvitation {
        let response = try await repository.issueInvitation(groupSpaceId: groupSpaceId)
    ...
    func list(groupSpaceId: Int64) async throws -> [GroupInvitationSummary] {
        try await repository.fetchInvitations(groupSpaceId: groupSpaceId).invitations.map {
    ...
    func preview(type: GroupInvitationCredentialKind, credential: String) async throws -> GroupInvitationPreview {
        let response = try await repository.previewInvitation(.init(credentialType: type.previewType, credential: credential))
    ...
    func accept(type: GroupInvitationCredentialKind, credential: String, nickname: String) async throws -> GroupInvitationAcceptanceResult {
        let response = try await repository.acceptInvitation(.init(credentialType: type.acceptanceType, credential: credential, nickname: nickname))
