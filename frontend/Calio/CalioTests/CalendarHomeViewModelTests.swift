...
struct CalendarHomeViewModelTests {
  ...
  @Test func calendarHomeViewModelSetReferenceDayInsideLoadedRangePreservesDateCells() async throws
  {
    ...
    #expect(
      viewModel.state.daysByKey[DayKey(date: baseDate, calendar: calendar)]?.events.map(\.backendId)
        == [42])
  }
  ...
  @Test func calendarHomeViewModelInitialLoadRequestsReferenceAndAdjacentMonthsOnly() async throws {
    ...
    #expect(
      repository.requestMonthKeys(calendar: calendar) == [
  ...
  @MainActor
  @Test func calendarHomeViewModelSelectYearMonthJumpsImmediatelyWithoutIntermediateFetches()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(calendar.date(from: DateComponents(year: 2026, month: 6, day: 8)))
  ...
  @MainActor
  @Test func calendarHomeViewModelDiscardsEventResponseStartedBeforeGlobalCacheInvalidation()
    async throws
  {
    let calendar = fixedCalendar
    ...
    #expect(
      await waitUntil {
        viewModel.state.monthEventCache[juneKey]?.loadedEvents.map(\.backendId) == [
          freshEvent.backendId
        ]
      })
    repository.finishSuspendedRequests()
  }
  ...
  @MainActor
  @Test func calendarHomeViewModelRefreshesVisibleMonthPrefetchRangeAfterAssistantResponse()
    async throws
  {
    let calendar = fixedCalendar
  ...
  @MainActor
  @Test func calendarHomeViewModelKeepsReferenceAndExposesRetryStateWhenTargetMonthFails()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(calendar.date(from: DateComponents(year: 2026, month: 6, day: 8)))
  ...
  @Test func calendarHomeViewModelLoadsHolidaysIntoSeparateDateCellItems() async throws {
    ...
    #expect(
      holidayRepository.requestMonthKeys == [
        YearMonthKey(year: 2026, month: 5),
        YearMonthKey(year: 2026, month: 6),
        YearMonthKey(year: 2026, month: 7),
      ])
  ...
  @Test func calendarHomeViewModelHolidayFailureKeepsEventAreaUsable() async throws {
    ...
    #expect(
      viewModel.state.daysByKey[DayKey(date: baseDate, calendar: calendar)]?.holidays.isEmpty
        == true)
  }
  ...
  @Test func calendarHomeViewModelInsertsCreatedEventIntoStartAtMonthCache() async throws {
    ...
    let startAt = try #require(
      calendar.date(byAdding: DateComponents(day: 1, hour: 9), to: baseDate))
  ...
  @Test func calendarHomeViewModelDoesNotTreatCreatedEventInIdleMonthAsLoadedCache() async throws {
    ...
    let baseDate = try #require(calendar.date(from: DateComponents(year: 2026, month: 6, day: 8)))
    let startAt = try #require(
      calendar.date(byAdding: DateComponents(day: 5, hour: 9), to: baseDate))
  ...
  @MainActor
  @Test func calendarHomeViewModelDoesNotReplaceLoadingMonthCacheWhenCreatedEventArrives()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(calendar.date(from: DateComponents(year: 2026, month: 6, day: 8)))
    ...
    #expect(
      viewModel.state.daysByKey[DayKey(date: createdEvent.startAt, calendar: calendar)]?.events.map(
        \.backendId) == [101])
  }
  ...
  @MainActor
  @Test func calendarHomeViewModelClearsFailureOnRetryAndBlocksDuplicateCreateWhileSaving()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(calendar.date(from: DateComponents(year: 2026, month: 6, day: 8)))
  ...
  @MainActor
  @Test
  func calendarHomeViewModelRefetchesDefaultRangeAfterRecurrenceCreateWithoutSynthesizingEvents()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(calendar.date(from: DateComponents(year: 2026, month: 6, day: 8)))
    ...
    #expect(
      viewModel.state.daysByKey[DayKey(date: baseDate, calendar: calendar)]?.events.isEmpty == true)
  }

  @MainActor
  @Test func calendarHomeViewModelRefetchesOriginalAndUpdatedMonthsAfterSingleEventUpdate()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(calendar.date(from: DateComponents(year: 2026, month: 6, day: 8)))
    let updatedStartAt = try #require(
  ...
  @MainActor
  @Test func calendarHomeViewModelUpdatesOnlyIndependentEventImportanceAndRefetchesItsMonth()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(
      calendar.date(from: DateComponents(year: 2026, month: 6, day: 8, hour: 9)))
    ...
    let response = EventResponseDTO(
      ...
      importantEvent: true,
      tag: TagResponseDTO(
        id: event.tag.id, title: event.tag.title, colorCode: event.tag.colorCode,
  ...
  @MainActor
  @Test func calendarHomeViewModelDoesNotRequestImportanceMutationForRecurrenceOccurrence()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(
  ...
  @MainActor
  @Test func calendarHomeViewModelPreventsDuplicateImportanceMutationWhileRequestIsInFlight()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(
  ...
  @Test func calendarHomeViewModelKeepsImportanceStateUnchangedWhenMutationFails() async throws {
    ...
    #expect(repository.updateImportantEventRequests.count == 1)
    #expect(
      viewModel.state.monthEventCache[YearMonthKey(date: baseDate, calendar: calendar)]?
        .loadedEvents
  ...
  @Test func calendarHomeViewModelEventMutationDoesNotInvalidateLoadedHolidayCache() async throws {
    ...
    #expect(
      viewModel.state.monthHolidayCache[YearMonthKey(year: 2026, month: 6)]?.loadedHolidays.map(
        \.id) == [1])
  ...
  @MainActor
  @Test
  func calendarHomeViewModelRecurrenceOccurrenceUpdateSendsOriginStartAtAndRefetchesDefaultRange()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(calendar.date(from: DateComponents(year: 2026, month: 6, day: 8)))
  ...
  @MainActor
  @Test
  func calendarHomeViewModelRecurrenceSeriesUpdateComposesUTCDateTimeAndRefetchesDefaultRange()
    async throws
  {
    let calendar = fixedCalendar
    let baseDate = try #require(calendar.date(from: DateComponents(year: 2026, month: 6, day: 8)))
    ...
    let startTime = try #require(
      calendar.date(from: DateComponents(year: 1970, month: 1, day: 1, hour: 9)))
    let endTime = try #require(
      calendar.date(from: DateComponents(year: 1970, month: 1, day: 1, hour: 10, minute: 30)))
