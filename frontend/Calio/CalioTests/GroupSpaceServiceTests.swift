...
struct GroupSpaceServiceTests {
  @Test func servicePreservesNetworkDecodingAndBackendFailures() async {
    ...
    await expectFetchFailure(
      .backend(
        statusCode: 409,
        problem: .init(
          type: nil, title: "CONFLICT", status: 409, detail: nil, errorCode: "GROUP_SPACE_CONFLICT")
      ),
  ...
  @Test func serviceMapsGroupSpaceDTOsToAppModels() async throws {
    let repository = GroupSpaceRepositoryStub(fetchResponses: [
      [GroupSpaceRepositoryStub.sampleSpace()]
    ])
  ...
  @Test func serviceForwardsLifecycleAndMembershipOperationsToRepository() async throws {
    ...
    #expect(
      repository.operations == [
        "create", "update:7", "members:7", "transfer:7:11", "remove:7:11", "leave:7", "delete:7",
      ])
  }
  ...
  @MainActor @Test func listViewModelPreservesConfirmedSpacesWhenRefreshFails() async {
    ...
    let repository = GroupSpaceRepositoryStub(
      fetchResponses: [[confirmedSpace], []], fetchErrorOnCall: 2)
  ...
  @MainActor @Test func listViewModelShowsInitialFailureThenRecoversOnRetry() async {
    ...
    let repository = GroupSpaceRepositoryStub(
      fetchResponses: [[confirmedSpace]], fetchErrorOnCall: 1)
    let viewModel = GroupSpaceListViewModel(service: GroupSpaceService(repository: repository))
  ...
  @MainActor @Test func listViewModelExposesCreationFailureEvenAfterInitialLoadFailure() async {
    let viewModel = GroupSpaceListViewModel(
      service: GroupSpaceService(
        repository: FailingGroupSpaceRepository(
          error: .backend(
            statusCode: 409,
            problem: .init(
              type: nil, title: "CONFLICT", status: 409, detail: nil,
              errorCode: "GROUP_SPACE_CONFLICT")
          )))
    )
  ...
  @MainActor @Test
  func invitationViewModelRetainsPreviewAfterAcceptanceErrorAndClearsItOnDismissal() async {
    let repository = GroupSpaceRepositoryStub(
      acceptError: GroupSpaceRepositoryStub.StubError.failed)
    ...
    let accepted = await viewModel.accept(
      type: .inviteCode, credential: "CALIO-2026", nickname: "준하")
  ...
  @MainActor @Test func invitationViewModelPublishesBackendConfirmedAlreadyMemberResult() async {
    let repository = GroupSpaceRepositoryStub(
      previewResponse: .init(
        name: "프로젝트 팀", emoji: "🗓️", memberCount: 2, expiresAt: GroupSpaceRepositoryStub.date),
      acceptResponse: .init(
        joinResult: .alreadyMember,
        groupSpace: .init(
          id: 7, name: "프로젝트 팀", emoji: "🗓️",
          myMembership: .init(memberId: 10, nickname: "준하", role: .member), memberCount: 2,
          createdAt: GroupSpaceRepositoryStub.date),
  ...
  {
    let repository = GroupSpaceRepositoryStub(
      fetchGroupSpaceError: .network(URLError(.notConnectedToInternet)))
    let viewModel = GroupSpaceDetailViewModel(
      groupSpace: GroupSpaceRepositoryStub.sampleGroupSpace(),
      service: GroupSpaceService(repository: repository)
    )
  ...
  @MainActor @Test func detailViewModelRecordsOperationFailuresWithoutDiscardingBackendErrorCode()
    async
  {
    let viewModel = GroupSpaceDetailViewModel(
      groupSpace: GroupSpaceRepositoryStub.sampleGroupSpace(),
      service: GroupSpaceService(
        repository: FailingGroupSpaceRepository(
          error: .backend(
            statusCode: 403,
            problem: .init(
              type: nil, title: "FORBIDDEN", status: 403, detail: nil,
              errorCode: "GROUP_SPACE_FORBIDDEN")
          )))
    )
...
private final class GroupSpaceRepositoryStub: GroupSpaceRepository {
  ...
  init(
    ...
    previewResponse: PreviewGroupInvitationResponseDTO = .init(
      name: "프로젝트 팀", emoji: nil, memberCount: 2, expiresAt: Date(timeIntervalSince1970: 0)),
    acceptResponse: AcceptGroupInvitationResponseDTO = .init(
      joinResult: .joined,
      groupSpace: .init(
        id: 7, name: "프로젝트 팀", emoji: nil,
        myMembership: .init(memberId: 10, nickname: "준하", role: .member), memberCount: 2,
  ...
  static func sampleSpace(name: String = "프로젝트 팀") -> GroupSpaceResponseDTO {
    .init(
      groupSpaceId: 7, name: name, emoji: nil, memberCount: 2,
      myMembership: .init(
        nickname: "준하", role: .owner, createdAt: date, updatedAt: date, statusChangedAt: date),
      createdAt: date, updatedAt: date)
  }

  static func sampleGroupSpace(name: String = "프로젝트 팀", role: GroupMemberRole = .owner)
    -> GroupSpace
  {
    GroupSpace(
      groupSpaceId: 7,
  ...
  func issueInvitation(groupSpaceId: Int64) async throws -> GroupInvitationResponseDTO {
    ...
    return .init(
      invitationId: 3, inviteUrl: "https://calio.app/invite/token", inviteCode: "CALIO-2026",
      expiresAt: Self.date)
  }
  ...
  func fetchInvitations(groupSpaceId: Int64) async throws -> GroupInvitationListResponseDTO {
    ...
    let invitations =
      invitationFetchResponses.isEmpty
      ? []
  ...
  func previewInvitation(_ request: PreviewGroupInvitationRequestDTO) async throws
    -> PreviewGroupInvitationResponseDTO
  {
  ...
  func acceptInvitation(_ request: AcceptGroupInvitationRequestDTO) async throws
    -> AcceptGroupInvitationResponseDTO
  {
...
extension GroupSpaceRepository {
  ...
  fileprivate func fetchInvitations(groupSpaceId: Int64) async throws
    -> GroupInvitationListResponseDTO
  {
    throw GroupSpaceRepositoryStub.StubError.failed
  }

  fileprivate func revokeInvitation(groupSpaceId: Int64, invitationId: Int64) async throws {
    throw GroupSpaceRepositoryStub.StubError.failed
  }

  fileprivate func previewInvitation(_ request: PreviewGroupInvitationRequestDTO) async throws
    -> PreviewGroupInvitationResponseDTO
  {
    throw GroupSpaceRepositoryStub.StubError.failed
  }

  fileprivate func acceptInvitation(_ request: AcceptGroupInvitationRequestDTO) async throws
    -> AcceptGroupInvitationResponseDTO
  {
    throw GroupSpaceRepositoryStub.StubError.failed
  }
...
private struct FailingGroupSpaceRepository: GroupSpaceRepository {
  ...
  func createGroupSpace(_ request: CreateGroupSpaceRequestDTO) async throws -> GroupSpaceResponseDTO
  { throw error }
  func updateGroupSpace(groupSpaceId: Int64, request: UpdateGroupSpaceRequestDTO) async throws
    -> GroupSpaceResponseDTO
  { throw error }
  ...
  func transferOwnership(groupSpaceId: Int64, request: TransferGroupOwnerRequestDTO) async throws
    -> TransferGroupOwnerResponseDTO
  { throw error }
