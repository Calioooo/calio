...
struct RecurrenceTests {
  ...
  @Test func createRecurrenceEventRequestDTOEncodesOnlyBackendContractFields() async throws {
    ...
    #expect(
      Set(object.keys) == [
        "title", "description", "allDay", "firstOccurrenceStartAt", "firstOccurrenceEndAt",
        "timeZone", "recurrence", "tagId",
      ])
  ...
  @Test func recurrenceRuleKeepsOnlyBoundedSimpleRulesEditable() async throws {
    ...
    #expect(
      RecurrenceRule.editableRule(from: ["RRULE:FREQ=MONTHLY"], allDay: false)
        == EditableRecurrenceRule(frequency: .monthly, until: nil))
    #expect(RecurrenceRule.editableRule(from: ["RRULE:FREQ=MONTHLY;"], allDay: false) == nil)
    #expect(
      RecurrenceRule.editableRule(
        from: ["RRULE:FREQ=MONTHLY;;UNTIL=20260831T000000Z"], allDay: false) == nil)
    #expect(RecurrenceRule.editableRule(from: ["RRULE:FREQ=WEEKLY;COUNT=4"], allDay: false) == nil)
    #expect(
      RecurrenceRule.editableRule(
        from: ["RRULE:FREQ=WEEKLY;UNTIL=20260831T000000Z", "EXDATE:20260817T000000Z"], allDay: false
      ) == nil)
  }
  ...
  @Test func eventServiceUpdatesUnboundedSeriesWithFrequencyOnlyRule() async throws {
    ...
    #expect(
      repository.updateRecurrenceEventRequests.first?.request.recurrence == ["RRULE:FREQ=MONTHLY"])
  }
  ...
  @Test func recurrenceInputRestoresOneYearBoundedDefaultAfterNoEndIsDisabled() throws {
    ...
    let expectedEndDate = try #require(
      calendar.date(from: DateComponents(year: 2027, month: 8, day: 1)))
  ...
  @Test func eventServicePreservesSeriesWallClockValuesForRecurrenceEditing() async throws {
    ...
    #expect(
      formCalendar.dateComponents([.year, .month, .day], from: details.recurrenceStartDate)
    ...
    #expect(
      formCalendar.dateComponents(
        [.year, .month, .day], from: try #require(details.recurrenceEndDate))
  ...
  @Test func recurrenceScheduleUsesSeriesZoneForDstAndInclusiveUntil() throws {
    ...
    let startTime = try #require(
      calendar.date(from: DateComponents(year: 2001, month: 1, day: 1, hour: 9)))
    let endTime = try #require(
      calendar.date(from: DateComponents(year: 2001, month: 1, day: 1, hour: 10)))
  ...
  @Test func eventServiceRecognizesAllDayRecurrenceRuleWithoutBackendFlag() async throws {
    ...
    #expect(
      CalendarDateService.localDateString(from: try #require(details.recurrenceEndDate))
        == "2026-08-31")
  }
  @Test func eventServiceCreateRecurrenceEventMapsSeparateUTCDateAndTimeIntoRepositoryRequest()
    async throws
  {
    var kstCalendar = Calendar(identifier: .gregorian)
    kstCalendar.timeZone = TimeZone(secondsFromGMT: 9 * 3600)!
    let recurrenceStartDate = try #require(
      kstCalendar.date(from: DateComponents(year: 2026, month: 8, day: 1, hour: 9)))
    let recurrenceEndDate = try #require(
      kstCalendar.date(from: DateComponents(year: 2026, month: 8, day: 31, hour: 9)))
    let recurrenceStartTime = try #require(
      kstCalendar.date(from: DateComponents(year: 2026, month: 9, day: 2, hour: 9)))
    let recurrenceEndTime = try #require(
      kstCalendar.date(from: DateComponents(year: 2026, month: 9, day: 2, hour: 10, minute: 30)))
    let repository = RecordingEventRepository()
    let service = EventService(repository: repository)
