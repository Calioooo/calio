...
final class RecordingEventRepository: EventRepository {
  ...
  private var storedUpdateImportantEventRequests:
    [(eventId: Int64, request: UpdateImportantEventRequestDTO)] = []
  private var storedUpdateRecurrenceEventRequests:
    [(recurrenceId: Int64, request: UpdateRecurrenceEventRequestDTO)] = []
  private var storedUpdateRecurrenceOccurrenceRequests:
    [(recurrenceId: Int64, request: UpdateRecurrenceOccurrenceRequestDTO)] = []
  private var storedDeleteEventIDs: [Int64] = []
  private var storedDeleteRecurrenceEventIDs: [Int64] = []
  private var storedDeleteRecurrenceOccurrenceRequests:
    [(recurrenceId: Int64, originStartAt: Date)] = []
  ...
  var updateRecurrenceEventRequests:
    [(recurrenceId: Int64, request: UpdateRecurrenceEventRequestDTO)]
  {
    locked {
  ...
  var updateRecurrenceOccurrenceRequests:
    [(recurrenceId: Int64, request: UpdateRecurrenceOccurrenceRequestDTO)]
  {
    locked {
  ...
  func finishSuspendedRequest(
    ...
    let request: SuspendedEventRequest? = locked {
      guard
        let index = suspendedContinuations.firstIndex(where: {
          YearMonthKey(date: $0.startDate, calendar: calendar) == key
        })
      else {
        return nil
  ...
  func finishSuspendedImportantEventRequests() {
    ...
    for continuation in continuations {
      continuation.resume(returning: updateResponse)
    }
  }
