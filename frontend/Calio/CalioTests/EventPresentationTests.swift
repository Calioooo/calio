...
struct EventPresentationTests {
  ...
  @Test func eventDisplayTextIncludesDatesOnlyForMultiDayRanges() async throws {
    ...
    #expect(
      sameDayText == CalendarEventDisplayText.timeRange(startAt: startAt, endAt: sameDayEndAt))
    #expect(
      multiDayText != CalendarEventDisplayText.timeRange(startAt: startAt, endAt: nextDayEndAt))
  ...
  @Test func eventDatePresentationUsesKoreanDatesWithoutChangingRangeSemantics() throws {
    ...
    let sameDayStart = try #require(
      calendar.date(from: DateComponents(year: 2026, month: 8, day: 17, hour: 10)))
    let sameDayEnd = try #require(
      calendar.date(from: DateComponents(year: 2026, month: 8, day: 17, hour: 11)))
    let nextDayEnd = try #require(
      calendar.date(from: DateComponents(year: 2026, month: 8, day: 18, hour: 11)))
    let allDayStart = try #require(
      calendar.date(from: DateComponents(year: 2026, month: 8, day: 17)))
    let allDayInclusiveEnd = try #require(
      calendar.date(from: DateComponents(year: 2026, month: 8, day: 18)))

    #expect(
      CalendarEventDisplayText.dateRange(
        startAt: sameDayStart, endAt: sameDayEnd, calendar: calendar) == "2026년 8월 17일")
    #expect(
      CalendarEventDisplayText.dateRange(
        startAt: sameDayStart, endAt: nextDayEnd, calendar: calendar)
        == "2026년 8월 17일 - 2026년 8월 18일")
    #expect(
      CalendarEventDisplayText.dateRange(
        startAt: allDayStart, endAt: allDayInclusiveEnd, calendar: calendar)
        == "2026년 8월 17일 - 2026년 8월 18일")
    #expect(
      CalendarEventDisplayText.compactDateTimeRange(
        startAt: sameDayStart, endAt: sameDayEnd, calendar: calendar)
