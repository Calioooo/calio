...
struct CalendarAssistantConversationTests {
    ...
    @Test func assistantBlockDecodingPreservesBackendOrderAndUnknownBlocks() throws {
        ...
        guard case .freeTimes(let first) = blocks[0] else { Issue.record("Expected first FREE_TIMES block"); return }
        #expect(first.first?.start == "2026-08-18T13:00:00+09:00")
        guard case .unsupported(let type) = blocks[1] else { Issue.record("Unknown blocks must remain visible to the client"); return }
        #expect(type == "FUTURE_BLOCK")
        guard case .freeTimes(let last) = blocks[2] else { Issue.record("Expected final FREE_TIMES block"); return }
        #expect(last.isEmpty)
    }
    ...
    @Test func createMutationPreviewDecodesBackendPersonalDefaultTag() throws {
        ...
        let response = try APIJSONCoding.makeDecoder().decode(SendCalendarConversationMessageResponseDTO.self, from: data)

        guard case .mutationPreviews(let previews) = response.blocks.first,
              let preview = previews.first,
              let after = preview.after else {
    ...
    @Test func repositoryUsesAuthenticatedConversationPathsAndTimezonePayload() async throws {
        let response = Data("{\"conversationId\":\"conversation-7\",\"assistantMessage\":\"확인했어요.\",\"blocks\":[]}".utf8)
        ...
        _ = try await repository.sendMessage(conversationId: "conversation-7", request: .init(message: "회의 옮겨줘", timeZone: "Asia/Seoul"))

        #expect(requests.map { $0.url?.path } == ["/api/ai/calendar/conversations", "/api/ai/calendar/conversations/conversation-7/messages"])
    ...
    @Test func servicePreservesNaturalLanguageApprovalAndMapsBlocksInOrder() async throws {
        let repository = CalendarConversationRepositoryStub(
            ...
            responses: [
                .init(
                    ...
                    blocks: [
                        .freeTimes([.init(start: "2026-09-04T13:00:00+09:00", end: "2026-09-04T14:00:00+09:00", allDayNotices: [])]),
        ...
        #expect(repository.sentMessages == [.init(conversationId: "conversation-7", message: "네, 이 제안을 적용해줘", timeZone: "Asia/Seoul")])
    ...
    @MainActor @Test func viewModelCreatesFreshSessionAndOnlyRefreshesAfterSuccessfulMessage() async {
        let repository = CalendarConversationRepositoryStub(conversationIds: ["first", "second"], responses: [.init(conversationId: "first", assistantMessage: "처리했어요.", blocks: [])])
        var refreshCount = 0
        let first = CalendarAssistantConversationViewModel(service: .init(repository: repository)) { refreshCount += 1 }
        await first.start()
    ...
    @MainActor @Test func viewModelGatesEmptyComposerAndPassesApprovalThroughUnchanged() async {
        ...
        #expect(repository.sentMessages == [.init(conversationId: "conversation-7", message: "네, 이 제안을 적용해줘", timeZone: TimeZone.current.identifier)])
        #expect(viewModel.messages.map(\.role) == [.user, .assistant])
    }
    ...
    @MainActor @Test func viewModelKeepsTranscriptAndAllowsRetryAfterMessageFailure() async {
        let repository = CalendarConversationRepositoryStub(
            conversationIds: ["conversation-7"],
            responses: [.failure(CalendarAssistantFailure.message), .success(.init(conversationId: "conversation-7", assistantMessage: "다시 처리했어요.", blocks: []))]
        )
        let viewModel = CalendarAssistantConversationViewModel(service: .init(repository: repository))
...
private final class CalendarConversationRepositoryStub: CalendarConversationRepository {
    ...
    func sendMessage(conversationId: String, request: SendCalendarConversationMessageRequestDTO) async throws -> SendCalendarConversationMessageResponseDTO {
        sentMessages.append(.init(conversationId: conversationId, message: request.message, timeZone: request.timeZone))
