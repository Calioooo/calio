...
struct APIClientTests {
  ...
  @Test func emptySuccessDoesNotDecodeBody() async throws {
    let client = APIClient(
      baseURL: try #require(URL(string: "https://example.test")), session: makeSession())
    TestURLProtocol.handler = { request in
  ...
  @Test func backendProblemDetailRetainsStatusAndErrorCode() async throws {
    ...
    TestURLProtocol.handler = { request in
      response(
        ...
        body:
          #"{"type":"about:blank","title":"VALIDATION_FAILED","status":400,"detail":"Validation failed.","errorCode":"VALIDATION_FAILED"}"#
      )
    }

    do {
      _ = try await client.send(
        APIClientTestResponse.self, method: .get, pathComponents: ["api", "events"],
        authorization: .bearer)
  ...
  @Test func networkAndSuccessDecodingFailuresRemainDistinct() async throws {
    ...
    TestURLProtocol.handler = { request in
      response(for: request, statusCode: 200, body: #"{"other":"field"}"#)
    }
    do {
      _ = try await client.send(
        APIClientTestResponse.self, method: .get, pathComponents: ["api", "events"],
  ...
  @Test func commonJSONCodingDecodesFractionalAndWholeSecondInstants() throws {
    ...
    let fractional = try decoder.decode(
      APIClientTestDateResponse.self,
      from: #"{"date":"2026-08-05T01:02:03.456Z"}"#.data(using: .utf8)!)
    let wholeSecond = try decoder.decode(
      APIClientTestDateResponse.self, from: #"{"date":"2026-08-05T01:02:03Z"}"#.data(using: .utf8)!)
  ...
  @Test func repositoriesUseExpectedEndpointAndAuthorizationPolicies() async throws {
    ...
    let eventRepository = URLSessionEventRepository(
      baseURL: baseURL, session: session, authTokenProvider: tokenProvider)
    ...
    TestURLProtocol.handler = { request in
      ...
      #expect(request.value(forHTTPHeaderField: "Authorization") == nil)
      return response(
        for: request, statusCode: 201, body: #"{"accessToken":"token","tokenType":"Bearer"}"#)
    }
...
private func response(for request: URLRequest, statusCode: Int, body: String) -> (
