...
struct VoteRoomViewModelTests {
  @Test @MainActor func restoresExistingSelectionAsSavedAndDraftState() async {
    ...
    #expect(viewModel.savedUnavailableDays == [VoteDay(year: 2026, month: 10, day: 16), VoteDay(year: 2026, month: 10, day: 17)])
...
private final class VoteRoomRepositoryStub: VoteRepository {
  ...
  init(
    ...
    self.resultResponse = resultResponse ?? VoteResultResponseDTO(
      publicId: UUID(), name: "투표", candidateStartDate: "2026-10-01", candidateEndDate: "2026-10-01",
  ...
  func createVoteRoom(_: CreateVoteRoomRequestDTO) async throws -> VoteRoomResponseDTO { fatalError() }
  ...
  func submitVotes(publicId _: UUID, request: SubmitVoteRequestDTO) async throws -> VoteSubmissionResponseDTO {
