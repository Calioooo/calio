...
struct CalendarScrollFocusCoordinatorTests {
  ...
  @Test func calendarScrollFocusCoordinatorPreparesReferenceDayBeforeRendering() async throws {
    ...
    #expect(
      !coordinator.canRenderContent(
        referenceDay: referenceDay, itemIDs: [earlierDay, referenceDay]))

    coordinator.prepareContentPosition(
      referenceDay: referenceDay, itemIDs: [earlierDay, referenceDay])

    #expect(coordinator.scrollPosition == referenceDay)
    #expect(
      coordinator.canRenderContent(referenceDay: referenceDay, itemIDs: [earlierDay, referenceDay]))
  }

  @MainActor
  @Test
  func calendarScrollFocusCoordinatorIgnoresInitialNonReferencePositionThenNotifiesUserScroll()
    async throws
  {
    let referenceDay = DayKey(year: 2026, month: 6, day: 23)
    let earlierDay = DayKey(year: 2026, month: 4, day: 23)
    ...
    coordinator.prepareContentPosition(
      referenceDay: referenceDay, itemIDs: [earlierDay, referenceDay, nextDay])
  ...
  @MainActor
  @Test func calendarScrollFocusCoordinatorKeepsContinuousUserScrollAfterReferenceChange()
    async throws
  {
    let referenceDay = DayKey(year: 2026, month: 6, day: 23)
    let nextDay = DayKey(year: 2026, month: 6, day: 24)
