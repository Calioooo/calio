...
struct CalendarViewTests {
  ...
  @Test func monthEventLayoutCreatesSingleSpanForMultiDayEventInSameWeek() async throws {
    let calendar = fixedCalendar
    let monthStartDate = try #require(
    ...
    let endAt = try #require(
      calendar.date(from: DateComponents(year: 2026, month: 6, day: 6, hour: 10)))
    let event = makeEvent(
      id: 30,
      title: "여러 날짜 일정",
      startAt: startAt,
    ...
    let itemDays = [3, 4, 5, 6].map { day -> CalendarDayItem in
      let date =
        calendar.date(from: DateComponents(year: 2026, month: 6, day: day)) ?? monthStartDate
    ...
    let item = makeDateCellItem(
      ...
      events: [
        makeEvent(
          ...
          title: "첫 일정",
          startAt: try #require(
            calendar.date(from: DateComponents(year: 2026, month: 6, day: 3, hour: 7))),
          endAt: try #require(
            calendar.date(from: DateComponents(year: 2026, month: 6, day: 3, hour: 9)))
        ),
        makeEvent(
          ...
          title: "두 번째 일정",
          startAt: try #require(
            calendar.date(from: DateComponents(year: 2026, month: 6, day: 3, hour: 8))),
          endAt: try #require(
    ...
    let events = [
      ...
      makeEvent(
        ...
        startAt: try #require(
          calendar.date(from: DateComponents(year: 2026, month: 6, day: 3, hour: 8))),
        endAt: try #require(
          calendar.date(from: DateComponents(year: 2026, month: 6, day: 3, hour: 11)))
      ),
      makeEvent(
        id: 3,
        title: "세 번째 일정",
